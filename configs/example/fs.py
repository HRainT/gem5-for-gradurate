    ...
    if args.xiangshan_system:
        for cpu in test_sys.cpu:
            cpu.mmu.pma_checker = PMAChecker(
                uncacheable=[AddrRange(0, size=0x40000000)])
...
if buildEnv["USE_ARM_ISA"] and not args.bare_metal \
...
if args.uop_cache:
    test_sys.useUopCache = True
else:
    test_sys.useUopCache = False

if args.vset_branch:
    test_sys.vsetBranch = True
else:
    test_sys.vsetBranch = False
    
if args.loop_buffer:
    test_sys.useLoopBuffer = True
else:
    test_sys.useLoopBuffer = False
    
if args.Ltage:
    test_sys.useLtage = True
else:
    test_sys.useLtage = False

if args.TournamentBP:
    test_sys.useTournamentBP = True
else:
    test_sys.useTournamentBP = False
    
if args.LocalBP:
    test_sys.useLocalBP = True
else:
    test_sys.useLocalBP = False  

if args.BiModeBP:
    test_sys.useBiModeBP = True
else:
    test_sys.useBiModeBP = False 
    
if args.rotating:
    test_sys.useRotating = True
else:
    test_sys.useRotating = False 

if args.loop_pc:
    if isinstance(args.loop_pc, str):
        test_sys.loopPC = int(args.loop_pc, base=16)
    else:
        test_sys.loopPC = 0x70003304
else:
    test_sys.loopPC = 0x70003304

if args.Specific_PC:
    if isinstance(args.Specific_PC, str):
        test_sys.SpecificPC = int(args.Specific_PC, base=16)
    else:
        test_sys.SpecificPC = 0x70003304
else:
    test_sys.SpecificPC = 0x70003304

if args.loopend_pc:
    if isinstance(args.loopend_pc, str):
        test_sys.loopendPC = int(args.loopend_pc, base=16)
    else:
        test_sys.loopendPC = 0x70003304
else:
    test_sys.loopendPC = 0x70003304

if args.loopstart_pc:
    if isinstance(args.loopstart_pc, str):
        test_sys.loopstartPC = int(args.loopstart_pc, base=16)
    else:
        test_sys.loopstartPC = 0x70003304
else:
    test_sys.loopstartPC = 0x70003304
...
test_sys.ucWidth = args.ucWidth
test_sys.BPUDelay = args.bpu_delay

if args.rxu_rename:
    test_sys.rxu_rename = True
else:
    test_sys.rxu_rename = False

if args.ideal_rotating:
    test_sys.idealRotating = True
else:
    test_sys.idealRotating = False
