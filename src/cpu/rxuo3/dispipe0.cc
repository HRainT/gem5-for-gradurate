    ...
    while (insts_available > 0 && macro_insts_available > 0 &&  send_insts < dispipe0Width) {
        ...
        if (cpu->rxu_rename) {
            ...
            if (renameStall[0] || renameStall[1]) {
                DPRINTF(RxuDispipe0,
                        "Blocking due to stall of rename vector csr.\n");
        ...
        const PCStateBase &next_pc, bool trace, InstSeqNum sn)
{
    ...
    DPRINTF(RxuDispipe0, "[tid:%i] [sn:%i] uop/eop created, inst is: \"%s\", opclass is: %s.\n",
            tid, sn,
...
Dispipe0::decodeVecInst(DynInstPtr &inst, InstQueue &uopBuffer)
{
    ...
    DPRINTF(RxuDispipe0,
            "[tid:%i] [sn:%i] vl: %d, Begin split uop from \"%s\", lmul: %d.\n",
            0,
            inst->seqNum,
            vl,
    ...
    while (1) {
        ...
        if (vl == 0) {
            ...
            instruction->ori_inst->allUopInVector = true;
        }

        uopBuffer.push_back(instruction);

        uop_list.push_back(instruction);
        if (!instruction->isVlff()) inst->MicroopQueue.push_back(instruction);
...
Dispipe0::calcFreeVecROBEntries(ThreadID tid)
{
    ...
    vecInstsInProgress[tid] = lastLastSendVecCount[tid] + lastLastSendVecCount[tid];
...
Dispipe0::setIidAndIbufferid(const DynInstPtr &inst)
{
    ...
    if (!inst->isMicroVector() && (inst->isMemRef() || inst->isReadBarrier() || inst->isWriteBarrier())) {
        if(ldst_cnt == 6) {
            inst->ibuffer_id = 6;
