...
LSQUnit::LSQUnitStats::LSQUnitStats(statistics::Group *parent)
    : statistics::Group(parent),
      ...
      ADD_STAT(blockedByCache, statistics::units::Count::get(),
               ...
               "being blocked"),
      ADD_STAT(CacheNoPort, statistics::units::Count::get(),
               "Number of times an access to memory failed due to the cache "
               "bo port"),
      ...
      ADD_STAT(crossCacheline, statistics::units::Count::get(),
                 "Number of load that cross cache lines"),
      ADD_STAT(stqsize, statistics::units::Count::get(),
                 "Number of stqsize")
{
    loadToUse
        .init(0, 299, 1)
...
LSQUnit::read(LSQRequest *request, ssize_t load_idx)
{
    ...
    if((request->mainReq()->getVaddr())/64 != (request->mainReq()->getVaddr()+request->mainReq()->getSize() - 1)/64){
        stats.crossCacheline++;
    }
