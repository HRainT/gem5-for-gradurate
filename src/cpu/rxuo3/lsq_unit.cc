...
#include "debug/RxuLSQUnit.hh"
#include "debug/RxuLSQUnitload.hh"
...
void
LSQUnit::init(CPU *cpu_ptr, EW *ew_ptr, Dispipe3 *p3_ptr, const BaseRxuO3CPUParams &params,
        LSQ *lsq_ptr, unsigned id)
{
    ...
    DPRINTF(RxuLSQUnit, "Creating LSQUnit%i object.\n",lsqID);
...
LSQUnit::name() const
{
    if (MaxThreads == 1) {
        ...
        return dispipe3Stage->name() + ".lsq.thread" + std::to_string(lsqID);
    }
}
...
LSQUnit::LSQUnitStats::LSQUnitStats(statistics::Group *parent)
    : statistics::Group(parent),
      ...
      ADD_STAT(blockedByCache, statistics::units::Count::get(),
               ...
               "being blocked"),
      ADD_STAT(loadToUse_cycle, statistics::units::Count::get(),
               "Number of cycle latency between the "
               "first time a load is issued and its completion"),
      ADD_STAT(loadToUse_num, statistics::units::Count::get(),
               "Number of instruction for the time between "
               "first time a load is issued and its completion"),
...
LSQUnit::insertLoad(const DynInstPtr &load_inst)
{
    ...
    DPRINTF(RxuLSQUnit, "Inserting load PC %s, idx:%i [sn:%lli]\n",
            load_inst->pcState(), loadQueue.tail(), load_inst->seqNum);

    /* Grow the queue. */
    loadQueue.advance_tail();

    bool wroteSqit = false;
    load_inst->sqIt = storeQueue.end();
    // if (storeQueue.size() == 0) {
    //     load_inst->sqIt = storeQueue.end();
    // } else {
    //     for (auto it = storeQueue.end(); it != storeQueue.begin(); it--) {
    //         if (it->instruction() && (it->instruction()->seqNum < load_inst->seqNum)) {
    //             load_inst->sqIt = it;
    //             wroteSqit = true;
    //         }
    //     }
    //     if (!wroteSqit) load_inst->sqIt = storeQueue.begin();
    // }
...
LSQUnit::insertStore(const DynInstPtr& store_inst)
{
    ...
    DPRINTF(RxuLSQUnit, "Inserting store PC %s, idx:%i [sn:%lli]\n",
...
{
        DPRINTF(RxuLSQUnit, "LQ size: %d, #loads occupied: %d\n",
...
{
        DPRINTF(RxuLSQUnit, "SQ size: %d, #stores occupied: %d\n",
...
LSQUnit::checkSnoop(PacketPtr pkt)
{
    ...
    DPRINTF(RxuLSQUnit, "Got snoop for address %#x\n", pkt->getAddr());
    ...
    while (++iter != loadQueue.end()) {
        ...
        DPRINTF(RxuLSQUnit, "-- inst [sn:%lli] to pktAddr:%#x\n",
        ...
        if (force_squash ||
            ...
            if (ld_inst->possibleLoadViolation() || force_squash) {
                DPRINTF(RxuLSQUnit, "Conflicting load at addr %#x [sn:%lli]\n",
                ...
                DPRINTF(RxuLSQUnit, "HitExternal Snoop for addr %#x [sn:%lli]\n",
                        pkt->getAddr(), ld_inst->seqNum);
        ...
        const DynInstPtr& inst)
{
     ...
     */
    // loadIt = loadQueue.begin();

    while (loadIt != loadQueue.end()) {
        DynInstPtr ld_inst = loadIt->instruction();
        if (!ld_inst->effAddrValid() || ld_inst->strictlyOrdered() || inst->seqNum <= ld_inst->ldfindvio) {
            ++loadIt;
            continue;
        }
            ...
            if (inst->isLoad()) {
                ...
                if (ld_inst->hitExternalSnoop()) {
                    if (!memDepViolator ||
                        ...
                        DPRINTF(RxuLSQUnit, "Detected fault with inst [sn:%lli] "
                ...
                DPRINTF(RxuLSQUnit, "Found possible load violation at addr: %#x"
                ...
                DPRINTF(RxuLSQUnit,
                        ...
                        inst_eff_addr2);
                DPRINTF(RxuLSQUnit, "Detected fault with inst [sn:%lli] and "
                        "[sn:%lli] at address %#x\n",
                        inst->seqNum, ld_inst->seqNum, ld_eff_addr1);
                memDepViolator = ld_inst;
...
LSQUnit::executeLoad(const DynInstPtr &inst)
{
    ...
    DPRINTF(RxuLSQUnit, "Executing load PC %s, [sn:%lli]\n",
            inst->pcState(), inst->seqNum);

    DPRINTF(RxuLSQUnitload, "Executing load PC %s, [sn:%lli]\n",
            inst->pcState(), inst->seqNum);
    ...
    if (load_fault == NoFault && !inst->readMemAccPredicate()) {
        ...
        DPRINTF(RxuLSQUnit, "Processing instruction [sn:%lli] with "
            "PC %s, ibuffer_id = %i\n", inst->seqNum, inst->pcState(), inst->ibuffer_id);
        ewStage->instToCommit(inst);
        ewStage->activityThisCycle();
        return NoFault;
    }
...
LSQUnit::executeStore(const DynInstPtr &store_inst)
{
    ...
    DPRINTF(RxuLSQUnit, "Executing store PC %s [sn:%lli]\n",
    ...
    if(store_inst->cache_wait != 0){
        store_inst->cache_wait--;
    }
    else if(store_inst->tlb_wait != 0){
        store_inst->tlb_wait--;
    }
    else{
        store_inst->num_exe = store_inst->num_exe + 1;
    }
    Fault store_fault = store_inst->initiateAcc();

    if (store_inst->staticInst->vElemMask) {
        store_inst->stdIssued = true;
        store_inst->stdDataReady = true;
    }
    ...
    if (!store_inst->readPredicate()) {
        DPRINTF(RxuLSQUnit, "Store [sn:%lli] not executed from predication\n",
                store_inst->seqNum);
        store_inst->forwardOldRegs();
        return store_fault;
    }
    DynInstPtr store;
    // if(store_inst->seqNum == 27958){
    //     store = store_inst;
    // }
    // if(store_inst->seqNum >= 27958){
    //     DPRINTF(RxuLSQUnit,"[sn:%i]'s storequeue.size() == %x\n",
    //             store->seqNum, storeQueue[store->sqIdx].size());
    // }
        if (storeQueue[store_idx].size() == 0) {
            DPRINTF(RxuLSQUnit,"Fault on Store PC %s, [sn:%lli], Size = 0\n",
                    store_inst->pcState(), store_inst->seqNum);
            ...
            if (store_inst->isAtomic()) {
                ...
                DPRINTF(RxuLSQUnit, "Processing instruction [sn:%lli] with "
                    "PC %s, ibuffer_id = %i\n", store_inst->seqNum, store_inst->pcState(), store_inst->ibuffer_id);
                ewStage->instToCommit(store_inst);
                ewStage->activityThisCycle();
            }
    ...
    if((store_inst->if_exe_second && store_inst->num_exe == 2) || !store_inst->if_exe_second){
    ...
    if (store_inst->isStoreConditional() || store_inst->isAtomic()) {
        ...
        if(store_inst->really_storeExe && !store_inst->hasAddStoresToWB){
            ++storesToWB;
            store_inst->hasAddStoresToWB = true;
            DPRINTF(RxuLSQUnit, "storesToWB + 1, PC "
                "%s [sn:%lli]\n",
                store_inst->pcState(),
                store_inst->seqNum);
        }
...
LSQUnit::commitLoad()
{
    ...
    DPRINTF(RxuLSQUnit, "Committing head load instruction, PC %s\n",
            inst->pcState());
    ...
    if (!inst->isInstPrefetch() && !inst->isDataPrefetch()
        ...
        stats.loadToUse_num++;
        stats.loadToUse_cycle += cpu->ticksToCycles(inst->firstIssue - inst->lastWakeDependents);
        stats.loadToUse.sample(cpu->ticksToCycles(
                    inst->firstIssue - inst->lastWakeDependents));
    }
...
LSQUnit::commitStores(InstSeqNum &youngest_inst)
{
    ...
    for (auto& x : storeQueue) {
        ...
        if (!x.canWB()) {
            if (x.instruction()->seqNum > youngest_inst && !x.instruction()->isEop() ||
                x.instruction()->seqNum > (youngest_inst + 16) && x.instruction()->isEop()) {
                break;
            }
            DPRINTF(RxuLSQUnit, "Marking store as able to write back, PC "
                    "%s [sn:%lli]\n",
                    x.instruction()->pcState(),
                    x.instruction()->seqNum);

            x.canWB() = true;
            if((x.instruction()->really_storeExe || x.instruction()->staticInst->vElemMask) && !x.instruction()->hasAddStoresToWB){
                ++storesToWB;
                x.instruction()->hasAddStoresToWB = true;
            DPRINTF(RxuLSQUnit, "storesToWB + 1, PC "
                "%s [sn:%lli]\n",
                x.instruction()->pcState(),
                x.instruction()->seqNum);
        ...
        else if(x.instruction()->staticInst->vElemMask && !x.instruction()->hasAddStoresToWB){
            ++storesToWB;
            x.instruction()->hasAddStoresToWB = true;
            DPRINTF(RxuLSQUnit, "storesToWB + 1, PC "
                "%s [sn:%lli]\n",
...
{
    if (isStoreBlocked) {
        DPRINTF(RxuLSQUnit, "Writing back  blocked store\n");
    ...
    while (storesToWB > 0 &&
           ...
           storeWBIt->valid() &&
           storeWBIt->canWB() &&
           (storeWBIt->instruction()->ifstdready || storeWBIt->instruction()->isAtomic()
           || storeWBIt->instruction()->isStoreConditional() || storeWBIt->instruction()->staticInst->vElemMask) &&
           ((!needsTSO) || (!storeInFlight)) &&
           lsq->cachePortAvailable(false)) {

        if (isStoreBlocked) {
            DPRINTF(RxuLSQUnit, "Unable to write back any more stores, cache"
                    " is blocked!\n");
            break;
        }
        ...
        if (storeWBIt->size() == 0) {
            ...
            DPRINTF(RxuLSQUnit, "storeWBIt: %i,line 807\n",storeWBIt.idx());
            continue;
        }

        if (storeWBIt->instruction()->isDataPrefetch()) {
            storeWBIt++;
            DPRINTF(RxuLSQUnit, "storeWBIt: %i,line 813\n",storeWBIt.idx());
            continue;
        }
        ...
        if ((request->mainReq()->isLLSC() ||
            ...
            DPRINTF(RxuLSQUnit, "Store idx:%i PC:%s to Addr:%#x "
        ...
        DPRINTF(RxuLSQUnit, "D-Cache: Writing back store idx:%i PC:%s "
        ...
        if (inst->isStoreConditional()) {
            ...
            if (!success) {
                ...
                DPRINTF(RxuLSQUnit, "Store conditional [sn:%lli] failed.  "
                ...
                if (!storeQueue.empty()){
                    storeWBIt++;
                    DPRINTF(RxuLSQUnit, "storeWBIt: %i,line 879\n",storeWBIt.idx());
                }
                else
        ...
        if (request->mainReq()->isLocalAccess()) {
            ...
            completeStore(storeWBIt);
            storeWBIt++;
            DPRINTF(RxuLSQUnit, "storeWBIt: %i,line 897\n",storeWBIt.idx());
            continue;
        }
        /* Send to cache */
        ...
        if (request->isSent()) {
            ...
            DPRINTF(RxuLSQUnit, "D-Cache became blocked when writing [sn:%lli], "
                    "will retry later\n",
                    inst->seqNum);
        }
    }
...
{
    DPRINTF(RxuLSQUnit, "Squashing until [sn:%lli]!"
            "(Loads:%i Stores:%i)\n", squashed_num, loadQueue.size(),
            storeQueue.size());
    cpu->dispipe3.wtb.memDepUnit->depPred.squash(squashed_num,0);
    
    while (loadQueue.size() != 0 &&
            loadQueue.back().instruction()->seqNum > squashed_num) {
        DPRINTF(RxuLSQUnit,"Load Instruction PC %s squashed, "
                "[sn:%lli]\n",
                loadQueue.back().instruction()->pcState(),
                loadQueue.back().instruction()->seqNum);
         bool is_acq_rel = loadQueue.back().instruction()->isFullMemBarrier() &&
                                (loadQueue.back().instruction()->isLoad() ||
                                (loadQueue.back().instruction()->isStore() &&
                                    !loadQueue.back().instruction()->isStoreConditional()));

        // Remove the instruction from the dependency list.
        if (is_acq_rel ||
            (!loadQueue.back().instruction()->isNonSpeculative() &&
            !loadQueue.back().instruction()->isStoreConditional() &&
            !loadQueue.back().instruction()->isAtomic() &&
            !loadQueue.back().instruction()->isReadBarrier() &&
            !loadQueue.back().instruction()->isWriteBarrier())) {

            for (int src_reg_idx = 0;
                src_reg_idx < loadQueue.back().instruction()->numSrcRegs();
                src_reg_idx++)
            {
                PhysRegIdPtr src_reg =
                    loadQueue.back().instruction()->renamedSrcIdx(src_reg_idx);
                ...
                if (!loadQueue.back().instruction()->readySrcIdx(src_reg_idx) &&
                    !src_reg->isFixedMapping()) {
                    dispipe3Stage->rmu->dependGraph.remove(src_reg->flatIndex(),
                                    loadQueue.back().instruction());
                }
            }
        }

            for (int dest_reg_idx = 0;
                    dest_reg_idx < loadQueue.back().instruction()->numDestRegs();
                    dest_reg_idx++)
                {
                    PhysRegIdPtr dest_reg =
                        loadQueue.back().instruction()->renamedDestIdx(dest_reg_idx);
    ...
    while (storeQueue.size() != 0 &&
        ...
        DPRINTF(RxuLSQUnit,"Store Instruction PC %s squashed, "
                "idx:%i [sn:%lli]\n",
                storeQueue.back().instruction()->pcState(),
                storeQueue.tail(), storeQueue.back().instruction()->seqNum);
        bool is_acq_rel = storeQueue.back().instruction()->isFullMemBarrier() &&
                                (storeQueue.back().instruction()->isLoad() ||
                                (storeQueue.back().instruction()->isStore() &&
                                    !storeQueue.back().instruction()->isStoreConditional()));

        if (is_acq_rel ||
            (!storeQueue.back().instruction()->isNonSpeculative() &&
            !storeQueue.back().instruction()->isStoreConditional() &&
            !storeQueue.back().instruction()->isAtomic() &&
            !storeQueue.back().instruction()->isReadBarrier() &&
            !storeQueue.back().instruction()->isWriteBarrier())) {

            for (int src_reg_idx = 0;
                src_reg_idx < storeQueue.back().instruction()->numSrcRegs();
                src_reg_idx++)
            {
                PhysRegIdPtr src_reg =
                    storeQueue.back().instruction()->renamedSrcIdx(src_reg_idx);
                ...
                // overwritten.  The only downside to this is it
                // leaves more room for error.
                if (!storeQueue.back().instruction()->readySrcIdx(src_reg_idx) &&
                    !src_reg->isFixedMapping()) {
                    dispipe3Stage->rmu->dependGraph.remove(src_reg->flatIndex(),
                                    storeQueue.back().instruction());
                }
            }
        }

            for (int dest_reg_idx = 0;
                    dest_reg_idx < storeQueue.back().instruction()->numDestRegs();
                    dest_reg_idx++)
                {
                    PhysRegIdPtr dest_reg =
                        storeQueue.back().instruction()->renamedDestIdx(dest_reg_idx);
                    if (dest_reg->isFixedMapping()){
                        continue;
                    }
                    while(!dispipe3Stage->rmu->dependGraph.empty(dest_reg->flatIndex())) {
...
LSQUnit::storePostSend()
{
    if (isStalled() &&
        ...
        DPRINTF(RxuLSQUnit, "Unstalling, stalling store [sn:%lli] "
                "load idx:%li\n",
                stallingStoreIsn, stallingLoadIdx);
        stalled = false;
        stallingStoreIsn = 0;
        dispipe3Stage->replayMemInst(loadQueue[stallingLoadIdx].instruction());
    }
    ...
    storeWBIt++;
     DPRINTF(RxuLSQUnit, "storeWBIt: %i,line 1170\n",storeWBIt.idx());
}
...
LSQUnit::writeback(const DynInstPtr &inst, PacketPtr pkt)
{
    ...
    if (!inst->isExecuted()) {
        ...
        if (inst->fault == NoFault) {
            ...
            DPRINTF(RxuLSQUnit, "Not completing instruction [sn:%lli] access "
    ...
    ewStage->checkMisprediction(inst);
}
...
LSQUnit::completeStore(typename StoreQueue::iterator store_idx)
{
    ...
    DPRINTF(RxuLSQUnit, "storesToWB - 1, PC "
        "%s [sn:%lli]\n",
        store_idx->instruction()->pcState(),
        store_idx->instruction()->seqNum);
    ...
    if (store_idx == storeQueue.begin()) {
        ...
        ewStage->updateLSQNextCycle = true;
    }
    store_inst->hadCommitted = true;
    DPRINTF(RxuLSQUnit, "Completing store [sn:%lli], idx:%i, store head "
            "idx:%i\n",
...
LSQUnit::trySendPacket(bool isLoad, PacketPtr data_pkt)
{
    ...
    DPRINTF(RxuLSQUnit, "Memory request (pkt: %s) from inst [sn:%llu] was"
...
{
    DPRINTF(RxuLSQUnit, "Unit %p marking stale translations %d %d\n", this,
...
{
    DPRINTF(RxuLSQUnit, "Unit %p checking stale translations\n", this);
    for (auto& entry : storeQueue) {
        if (entry.valid() && entry.hasRequest()
    ...
    DPRINTF(RxuLSQUnit, "Unit %p found no stale translations\n", this);
    return false;
}
...
{
    if (isStoreBlocked) {
        DPRINTF(RxuLSQUnit, "Receiving retry: blocked store\n");
        writebackBlockedStore();
    }
}
...
LSQUnit::read(LSQRequest *request, ssize_t load_idx)
{
    ...
    DPRINTF(RxuLSQUnit, "request: size: %u, Addr: %#lx\n",
    ...
    //     DPRINTF(RxuLSQUnit, "request: size: %u, Addr: %#lx, code: %d\n",
    ...
    // }
    load_inst->load_once = request->mainReq()->getVaddr();
    ...
    if (request->mainReq()->isStrictlyOrdered() &&
        ...
        // Tell WTB/mem dep unit that this instruction will need to be
        // rescheduled eventually
        dispipe3Stage->rescheduleMemInst(load_inst);
        load_inst->clearIssued();
        load_inst->effAddrValid(false);
        ++stats.rescheduledLoads;
        DPRINTF(RxuLSQUnit, "Strictly ordered load [sn:%lli] PC %s\n",
    ...
    DPRINTF(RxuLSQUnit, "Read called, load idx: %i, store idx: %i, "
            "storeHead: %i addr: %#x%s\n",
            load_idx - 1, load_inst->sqIt._idx, storeQueue.head() - 1,
            request->mainReq()->getPaddr(), request->isSplit() ? " split" :
            "");

    DPRINTF(Spike, "Execute load instruction with seqNum [%llu] and addr: %#x\n",
            load_inst->seqNum, request->mainReq()->getPaddr());
    ...
    for(auto ld_once_it = cpu->prefetch_load.begin();ld_once_it != cpu->prefetch_load.end();ld_once_it++){
        auto req_s = request->mainReq()->getVaddr();
        auto req_e = req_s + request->mainReq()->getSize();
        auto ld_once_s = ld_once_it->first;
        auto ld_once_e = ld_once_s + 0x16;
        bool ld_once_has_lower_limit = req_s >= ld_once_s;
        bool ld_once_has_upper_limit = req_e <= ld_once_e;
        if(ld_once_has_lower_limit && ld_once_has_upper_limit){
            memcpy(load_inst->memData,
                        &ld_once_it->second,
                        0x16);     

            DPRINTF(RxuLSQUnit, "Forwarding from ldonce to load to addr %#x\n", ld_once_it->first);
        }
    ...
    //while (store_it->instruction()->seqNum < load_inst->seqNum && !load_inst->isDataPrefetch() && store_it->instruction()->isCompleted()) {
        // Move the index to one younger
        store_it--;
        assert(store_it->valid());
        if(store_it->instruction()->seqNum > load_inst->seqNum){
            continue;
        }
        ...
        if (store_size != 0 && !store_it->instruction()->strictlyOrdered() &&
            ...
            DPRINTF(RxuLSQUnit, "req_s:%x,req_e:%x,st_s:%x,st_e:%x\n", req_s,
                    req_e, st_s, st_e);
            DPRINTF(RxuLSQUnit, "store_size:%x,store_pc:%s,req_size:%x,req_pc:%s",
            ...
            if (coverage == AddrRangeCoverage::FullAddrRangeCoverage) {
                ...
                DPRINTF(RxuLSQUnit, "Forwarding from store idx %i to load to "
                        "addr %#x\n", store_it._idx,
                ...
                // complete.
                DPRINTF(RxuLSQUnit, "Load-store forwarding mis-match. "
    ...
    DPRINTF(RxuLSQUnit, "Doing memory access for inst [sn:%lli] PC %s\n",
            load_inst->seqNum, load_inst->pcState());
    // if(load_inst->staticInst->opClass() == SimdVLoadOnceOp){
    //     lsq->prefetch_load.push_back(*request);
    // }
    ...
    request->sendPacketToCache();
    if (!request->isSent())
        dispipe3Stage->blockMemInst(load_inst);
...
LSQUnit::write(LSQRequest *request, uint8_t *data, ssize_t store_idx)
{
    ...
    DPRINTF(RxuLSQUnit, "Doing write to store idx %i, addr %#x | storeHead:%i "
            "[sn:%llu]\n",
            store_idx - 1, request->req()->getPaddr(), storeQueue.head() - 1,
            storeQueue[store_idx].instruction()->seqNum);

    DPRINTF(Spike, "Execute store instruction with seqNum [%llu] and addr: %#x\n",
            storeQueue[store_idx].instruction()->seqNum, request->req()->getPaddr());
