...
Predisq::tick()
{
    ...
    while (threads != end) {
        ...
        if (csrFenceInfo[tid].inst && csrFenceInfo[tid].inst->readyToCommit() && csrFenceInfo[tid].inst->inst_Real_Ready) {
...
Predisq::InsertInsts(ThreadID tid)
{
    ...
    if (compressFlag) {
        for (int i = 0; i < insts_available; i++) {
            ...
            // if(inst->fromWhichContainer[0]){
            //     stats.fetch_latency1++;
            // }
            // else if(inst->fromWhichContainer[1]){
            //     stats.fetch_latency2++;
            // }
            // else if(inst->fromWhichContainer[2]){
