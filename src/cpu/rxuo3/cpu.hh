...
class CPU : public BaseCPU
{
  ...
  public:
    ...
    InstSeqNum getAndIncrementInstSeq(bool incrementVector, bool incrementSplitMacro) {
      if (incrementVector && !incrementSplitMacro) {
        globalSeqNum += 40;
      }
      else if(incrementSplitMacro){
        globalSeqNum += 140;
      } 
      else {
        globalSeqNum++;
  ...
  public:
    ...
    private:
        int maxEntryNum;
        std::map<int, mergeBufferEntry> Buffer;
    ...
    public:
        ...
        mergeBufferEntry checkAndFetchEntry() {
            ...
            if (Buffer.begin()->second.remainUnmergeEop == 0) {
                mergeBufferEntry entry = Buffer.begin()->second;
        ...
        RiscvISA::VecRegContainer *fetchEntry(InstSeqNum sn) {
            ...
            // DPRINTF(RxuO3CPU, "[%lli] Uop's eop write data to merge buffer\n", sn);
            DPRINTF(RxuO3CPU, "[sn:%lli] Uop's eop write data to merge buffer, old data: %s\n", sn, reverseByteOrder(data->getString(16)));
            return data;
        }
