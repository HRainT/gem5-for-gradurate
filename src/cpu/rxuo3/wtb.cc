...
WTB::SpecialMacroReady::SpecialMacroReady(WTB *wtb_ptr, DynInstPtr &_inst)
...
void
WTB::SpecialMacroReady::process()
{
    wtbPtr->ibufferToEW.push(inst);
}

WTB::wakeUpVsetvli::wakeUpVsetvli(WTB *wtb_ptr, DynInstPtr &_inst)
    : Event(Stat_Event_Pri, AutoDelete),
      wtbPtr(wtb_ptr), inst(_inst)
{
}
...
void
WTB::wakeUpVsetvli::process()
{
    wtbPtr->processWakeUpVsetvli(inst);
    inst = NULL;
}
...
WTB::WTB(CPU *cpu_ptr, Dispipe3 *p3_ptr,
    ...
    : cpu(cpu_ptr),
      ...
      ldstOddRegReadNums(params.ldstOddRegReadNums),
      ldstEvenRegReadNums(params.ldstEvenRegReadNums),
      ...
      vectorReadRegNums(params.vectorReadRegNums),
      vectorLDSTReadRegNums(params.vectorLDSTReadRegNums),
      ...
      stats(cpu)
{
    ...
    for (int i = 0; i < 30; i++) {
        ...
        wbNumsRecord.intNormal_newEvenWbNums = 1;
        wbNumsRecord.intNormal_newOddWbNums = 1;
    ...
    vSpecialBuffer = new VSpecialBuffer(this);
...
WTB::WTBStats::WTBStats(CPU *cpu)
    ...
    ADD_STAT(earlywakeupvsetvli, statistics::units::Count::get(),
                "inst wakeupvsetvli"),
    ADD_STAT(readywakeupvsetvli, statistics::units::Count::get(),
                "inst readywakeupvsetvli")
{
    instsIssued
        .prereq(instsIssued);
    ...
    earlywakeupvsetvli
        .prereq(earlywakeupvsetvli);

    readywakeupvsetvli
        .prereq(readywakeupvsetvli);
}
...
WTB::VSpecialBuffer::VSpecialBuffer(WTB *wtb_ptr)
    : wtbPtr(wtb_ptr)
{
}

void
WTB::VSpecialBuffer::insertInst(DynInstPtr inst)
{
    insts.push_back(inst);
}

DynInstPtr
WTB::VSpecialBuffer::selectInst()
{
    if (insts.empty() || lastIssueInst && 
    !lastIssueInst->isExecuted() && !lastIssueInst->isSquashed()) return nullptr;

    for (auto it = insts.begin(); it != insts.end(); it++) {
        DynInstPtr inst = (*it);
        DPRINTF(RxuWTB,
                "Special Macro instruction [sn:%i] specialUopRdyNum: %d\n",
                inst->seqNum, inst->specialUopRdyNum);
        if (inst->specialUopRdyNum == inst->actualUopNumber) {
            insts.erase(it);
            lastIssueInst = inst;
            return inst;
        }
...
bool
WTB::VSpecialBuffer::entryEnough()
{
    return insts.size() + 4 <= max_size;
}

int
WTB::VSpecialBuffer::size()
{
    return insts.size();
}

void
WTB::VSpecialBuffer::squash(InstSeqNum sn)
{
    int size = insts.size();
    for (int i = 0; i < size; i++) {
        DynInstPtr inst = insts.front();
        insts.pop_front();
        if (inst->seqNum > sn) {
            if (!inst->isSquashed()) {
                inst->setSquashed();
                DPRINTF(RxuWTB,
                        "[tid:0] "
                        "Special Macro instruction [sn:%i] with PC %s is squashed in vSpecialBuffer->\n",
                        inst->seqNum, inst->pcState());
            } else {
                DPRINTF(RxuWTB,
                        "[tid:0] "
                        "Special Macro instruction [sn:%i] with PC %s "
                        "has already been squashed before in vSpecialBuffer->\n",
                        inst->seqNum, inst->pcState());
            }
        } else {
            insts.push_back(inst);
...
WTB::isFull(ThreadID tid, int index)
{
    if (index == 2 || index == 3) {
        ...
        else if(ibuffer[tid][6].size() >= LdStEntries){
            return true;
        }
        else if(ibuffer[tid][7].size() >= LdStEntries){
            return true;
        }
        else {
            return false;
    ...
    if (index == 2 || index == 3) {
        ...
        if(ismemRef){
            if(isStore){
                ...
                for (auto it = ibuffer[tid][6].begin(); it != ibuffer[tid][6].end(); it++)
                {
                    DPRINTF(RxuWTB, "ldst ibuffer6 full,inst:[sn:%lli].\n",(*it)->seqNum);
                }
                
                return true;
            }
            else if(ibuffer[tid][7].size() >= LdStEntries){
                for (auto it = ibuffer[tid][7].begin(); it != ibuffer[tid][7].end(); it++)
                {
                    DPRINTF(RxuWTB, "ldst ibuffer6 full,inst:[sn:%lli].\n",(*it)->seqNum);
                }
                return true;
...
{
    if (!cpu->vsetBranch) {
        if (new_inst->isVsetvli() && !cpu->vecCsr.isReady(Vl, new_inst->rn_vl) &&
            cpu->rmu.regScoreboard[new_inst->renamedSrcIdx(0)->flatIndex()]) {
            wakeUpVsetvli *wakeUpReady = new wakeUpVsetvli(this, new_inst);
            cpu->schedule(wakeUpReady, cpu->clockEdge(Cycles(1)));
            new_inst->hasReadyVl = true;
            DPRINTF(RxuWTB, "insert wake vl.\n");
            stats.readywakeupvsetvli++;
        }
    }
    ...
    // if(!new_inst->isSpecialVector()) dispipe3Stage->rmu->addToDependents(new_inst);

    if (new_inst->isStore() || new_inst->isAtomic()) {
        DPRINTF(RxuMemDepUnit, "Inserting store/atomic PC %s [sn:%lli].\n",
                new_inst->pcState(), new_inst->seqNum);
        memDepUnit[new_inst->threadNumber].stq.emplace(new_inst->seqNum,new_inst);
        memDepUnit[new_inst->threadNumber].depPred.insertStore(new_inst->pcState().instAddr(), new_inst->seqNum,
                new_inst->threadNumber);
    } 
            ...
            for (auto it = ibuffer[0][i].begin(); it != ibuffer[0][i].end(); ++it) {
                ...
                if((*it)->unlocaldata == false && (cpu->ticksToCycles(curTick()-(*it)->arrive_wtb)) > 3 && (*it)->arrive_wtb != -1){
                    ...
                    if((*it)->ibuffer_id == 22 || (*it)->ibuffer_id == 23){
                        for (int i = 0; i < (*it)->numSrcRegs(); i++) {
                            if ((*it)->src_need_wake[i]) {
                                if(vectorReadRegFreeNums > 0){
                                    vectorReadRegFreeNums--;
                                }
                                else {
                                    DPRINTF(RxuWTB, "inst[sn:%llu] lock Rport the circle arrive 3.\n",(*it)->seqNum);
                                    ++stats.fnormal_lockRpork;
                    ...
                    if((*it)->ibuffer_id == 7 || (*it)->ibuffer_id == 6 || (*it)->ibuffer_id == 14){
                        if((*it)->isFloating()){
                            for (int i = 0; i < (*it)->numSrcRegs(); i++) {
                                if ((*it)->src_need_wake[i]) {
                                    if(wheOld && (ldstOddRegReadFreeNums+fstRegReadFreeNums > 0)){
                                        if(fstRegReadFreeNums >0){
                                            fstRegReadFreeNums--;
                                        }
                                        else {
                                            ldstOddRegReadFreeNums--;
                                        }
                                    }
                                    else if(!wheOld && (ldstEvenRegReadFreeNums+fstRegReadFreeNums > 0)){
                                        if(fstRegReadFreeNums >0){
                                            fstRegReadFreeNums--;
                                        }
                                        else {
                                            ldstEvenRegReadFreeNums--;
                                        }
                                    }
                                    else {
                                        DPRINTF(RxuWTB, "inst[sn:%llu] lock Rport the circle arrive 3.\n",(*it)->seqNum);
                            ...
                            for (int i = 0; i < (*it)->numSrcRegs(); i++) {
                                if ((*it)->src_need_wake[i] ) {
                                    if((*it)->srcRegIdx(i).regClass().type() == IntRegClass){
                                        if(wheOld && intNormalOddRegReadFreeNums > 0){
                                            intNormalOddRegReadFreeNums--;
                                        }
                                        else if(!wheOld && intNormalEvenRegReadFreeNums > 0){
                                    ...
                                    else if(vectorLDSTReadRegFreeNums > 0 && i==1){
                                        vectorLDSTReadRegFreeNums--;
                                    }
                                    else if(vectorReadRegFreeNums > 0 && i==2){
                                        vectorReadRegFreeNums--;
                                    }
                                    else if(vectorLDSTReadRegFreeNums > 0 && i==3){
                                        vectorLDSTReadRegFreeNums--;
                                    }
                                    else {
                                        DPRINTF(RxuWTB, "inst[sn:%llu] lock Rport the circle arrive 3.\n",(*it)->seqNum);
                            ...
                            for (int i = 0; i < (*it)->numSrcRegs(); i++) {
                                if ((*it)->src_need_wake[i]) {
                                    ...
                                    // }
                                    // else 
                                    if(wheOld && (ldstOddRegReadFreeNums > 0)){                              
                                        ldstOddRegReadFreeNums--;
                                    }
                                    else if(!wheOld && (ldstEvenRegReadFreeNums >    0)){
                                        ldstEvenRegReadFreeNums--;
                                    }
                                    else {
                                        DPRINTF(RxuWTB, "inst[sn:%llu] lock Rport the circle arrive 3.\n",(*it)->seqNum);
    ...
    if(new_inst->ibuffer_id == 7 || new_inst->ibuffer_id == 6 || new_inst->ibuffer_id == 14){
        if(new_inst->isFloating()){
            for (int i = 0; i < new_inst->numSrcRegs(); i++) {
                if (new_inst->readySrcIdx(i) && !new_inst->had_read_RR[i]) {
                    if(wheOE_src && (ldstOddRegReadFreeNums+fstRegReadFreeNums >0)){
                        if(fstRegReadFreeNums >0){
                            fstRegReadFreeNums--;
                        }
                        else {
                    ...
                    else if(!wheOE_src && (ldstEvenRegReadFreeNums+fstRegReadFreeNums >0)){
                        if(fstRegReadFreeNums >0){
                            fstRegReadFreeNums--;
                        }
                        else {
            ...
            for (int i = 0; i < new_inst->numSrcRegs(); i++) {
                if (new_inst->readySrcIdx(i) && !new_inst->had_read_RR[i]) {
                    if(new_inst->srcRegIdx(i).regClass().type() == IntRegClass){
                        if(wheOE_src && intNormalOddRegReadFreeNums > 0){
                            intNormalOddRegReadFreeNums--;
                            new_inst->had_read_RR[i] = true;
                            new_inst->had_src_data = true;
                    ...
                    else if((new_inst->staticInst->machInst.vm && (i == (new_inst->numSrcRegs()-1))) && new_inst->isLoad()){
                        new_inst->had_read_RR[i] = true;                          
                        new_inst->had_src_data = true;
                    }
                    else if((!new_inst->staticInst->machInst.vm && (i == (new_inst->numSrcRegs()-2))) && new_inst->isLoad()){
                        new_inst->had_read_RR[i] = true;
                        new_inst->had_src_data = true;    
                    }
                    else if(vectorLDSTReadRegFreeNums > 0 && i==1 && new_inst->isLoad()){
                        vectorLDSTReadRegFreeNums--;
                        new_inst->had_read_RR[i] = true;                          
                        new_inst->had_src_data = true;
                    }
                    else if(vectorReadRegFreeNums > 0 && i==1 && new_inst->isStore()){
                        vectorReadRegFreeNums--;
                        new_inst->had_read_RR[i] = true;                          
                        new_inst->had_src_data = true;
                    }
            ...
            for (int i = 0; i < new_inst->numSrcRegs(); i++) {
                ...
                // else 
                if (new_inst->readySrcIdx(i) && !new_inst->had_read_RR[i]) {
                    if(wheOE_src && (ldstOddRegReadFreeNums>0)){
                        ldstOddRegReadFreeNums--;
                        new_inst->had_read_RR[i] = true;
                        new_inst->had_src_data = true;
                    }
                    else if(!wheOE_src && (ldstEvenRegReadFreeNums>0)){
                        ldstEvenRegReadFreeNums--;
                        new_inst->had_read_RR[i] = true;
                        new_inst->had_src_data = true;
                    }
...
WTB::scheduleReadyInsts()
{
    ...
    while ((mem_inst = getDeferredMemInstToExecute())) {
        mem_inst->tlb_wait++;
        mem_inst->inst_had_inReady = false;
        mem_inst->vld_had_issued = false;
    ...
    auto it_ld_rbk = ld_had_rbk.begin();
    while (it_ld_rbk != ld_had_rbk.end()) {
        if(ibuffer[0][14].empty()){
            if(readyLD_Dst_odd.size() <= readyLD_Dst_even.size()){
                readyLD_Dst_odd.push((*it_ld_rbk));
            }
            else {
                readyLD_Dst_even.push((*it_ld_rbk));
            }
            it_ld_rbk = ld_had_rbk.erase(it_ld_rbk);
            DPRINTF(RxuWTB, "inst had rbk seqnum:[sn:%i] to readylist \n",(*it_ld_rbk)->seqNum);
        }
        else if ((*it_ld_rbk)->seqNum < ibuffer[0][14].front()->seqNum) {
            if(readyLD_Dst_odd.size() <= readyLD_Dst_even.size()){
                readyLD_Dst_odd.push((*it_ld_rbk));
            }
            else {
    ...
    for (int i = 0; i < 24; i++) {
        ...
        // if(i==22 || i==23){
        //     for (auto it = ibuffer[0][i].begin(); it != ibuffer[0][i].end(); ++it) {
        //         DPRINTF(RxuWTB, "ibuffer[%i] [sn:%llu] [pc:%s] [inst:%s]\n", i, (*it)->seqNum, (*it)->pcState(), (*it)->staticInst->disassemble((*it)->pc->instAddr()));
        //     }
        // }
            }
    ...
    while (!ibufferToEW.empty()) {
        ...
        if (issuing_inst->opClass() == enums::IntDiv && pipelineUseNums.intDivNums <= 0) {               // indiv pipeline
            stats.statFuBusy[0]++;
            stats.fuBusyCycles++;
            ...
            continue;
        } else if (issuing_inst->opClass() == enums::FloatDiv && pipelineUseNums.fpDivNums <= 0) {            // fpdiv pipeline
            stats.statFuBusy[1]++;
            stats.fuBusyCycles++;
            ...
            continue;
        } else if (issuing_inst->opClass() == enums::FloatSqrt && pipelineUseNums.fpDivNums <= 0) {         // fpsqrt pipeline
            stats.statFuBusy[1]++;
            stats.fuBusyCycles++;
        ...
        if ((issuing_inst->ibuffer_id == 11 || issuing_inst->ibuffer_id == 12) &&
            ...
            if (issuing_inst->destRegIdx(0).classValue() == IntRegClass) {                // fp2int
                ...
                if (issuing_inst->odd_inst) {
                    if(wbNums.at(cpu->ew.fuPool->getOpLatency(issuing_inst->opClass()) + 1).intNormal_newOddWbNums >= 1){
                        wbNums.at(cpu->ew.fuPool->getOpLatency(issuing_inst->opClass()) + 1).intNormal_newOddWbNums--;
                    }
                    else if (wbNums.at(cpu->ew.fuPool->getOpLatency(issuing_inst->opClass()) + 1).intSpecialOddWbNums >= 1) {
                    ...
                    if(wbNums.at(cpu->ew.fuPool->getOpLatency(issuing_inst->opClass()) + 1).intNormal_newEvenWbNums >= 1){
                        wbNums.at(cpu->ew.fuPool->getOpLatency(issuing_inst->opClass()) + 1).intNormal_newEvenWbNums--;
                    }
                    else if (wbNums.at(cpu->ew.fuPool->getOpLatency(issuing_inst->opClass()) + 1).intSpecialEvenWbNums >= 1) {
                        if (issuing_inst->numDestRegs() > 0 && !issuing_inst->destRegIdx(0).isZeroReg())
                ...
                unsigned delay;
                if (issuing_inst->opClass() == enums::FloatDiv || issuing_inst->opClass() == enums::FloatSqrt) {
        ...
        // Check the write port of intSpecial
            ...
            if (issuing_inst->destRegIdx(0).classValue() == FloatRegClass) {                // int2fp
                ...
                unsigned delay;
                if (issuing_inst->opClass() == enums::IntDiv) {
                    delay = initFdivDelay + 1;
                ...
                if (issuing_inst->odd_inst) {
                    if(wbNums.at(delay).intNormal_newOddWbNums >= 1){
                        wbNums.at(delay).intNormal_newOddWbNums--;
                    }
                    else if (wbNums.at(delay).intSpecialOddWbNums >= 1) {
                        if (issuing_inst->numDestRegs() > 0 && !issuing_inst->destRegIdx(0).isZeroReg())
                    ...
                    if(wbNums.at(delay).intNormal_newEvenWbNums >= 1){
                        wbNums.at(delay).intNormal_newEvenWbNums--;
                    }
                    else if (wbNums.at(delay).intSpecialEvenWbNums >= 1) {
                        if (issuing_inst->numDestRegs() > 0 && !issuing_inst->destRegIdx(0).isZeroReg())
        ...
        } else if (issuing_inst->opClass() == enums::FloatDiv) {
        ...
        } else if (issuing_inst->opClass() == enums::FloatSqrt) {
        ...
        if(issuing_inst->isStore() && issuing_inst->staIssued && issuing_inst->ifstdready && issuing_inst->inst_to_ew
            ...
            Cycles op_latency;
            if (issuing_inst->opClass() == enums::IntDiv || issuing_inst->opClass() == enums::FloatDiv || issuing_inst->opClass() == enums::FloatSqrt) {
            ...
            if (issuing_inst->ibuffer_id != 6 && issuing_inst->ibuffer_id != 7 && issuing_inst->ibuffer_id != 14 && issuing_inst->ibuffer_id != 15 &&
                ...
                if (!issuing_inst->isSpecialVector()) {
                    if (op_latency == 1) {
                ...
                if(issuing_inst->stdNeedExeScd_issue && issuing_inst->num_exe ==1){
                    ...
                    cpu->schedule(wakeToIssue, cpu->clockEdge(Cycles(0)));
                }
                else{
                    LdstWakeToIssue *wakeToIssue = new LdstWakeToIssue(this, issuing_inst);
                    cpu->schedule(wakeToIssue, cpu->clockEdge(Cycles(0)));
...
WTB::addReadyMemInst(const DynInstPtr &inst)
{
    ...
    if (inst->ibuffer_id == 7 || inst->ibuffer_id == 15 || inst->ibuffer_id == 17 || inst->ibuffer_id == 19 ||
        ...
        if(inst->inst_had_inReady ==  false && (inst->had_src_data || inst->isStore())){
            ...
            if(inst->isStore()){
                ...
                // if(inst->isLoad() && cpu->dispipe3.wtb.memDepUnit->depPred.load_if_rbk(inst->getPC(),inst->seqNum)){
                //     ld_had_rbk.push_back(inst);
                //     DPRINTF(RxuWTB, "inst because of rbk add in ld_had_rbk,[sn:%lli] :%i\n", inst->seqNum);
                // }else {
                    if(readyLD_Dst_odd.size() <= readyLD_Dst_even.size()){
...
WTB::deferMemInst(const DynInstPtr &deferred_inst)
{
    ...
    if(deferred_inst->isStore()){
        memDepUnit->depPred.insertStore(deferred_inst->pcState().instAddr(), deferred_inst->seqNum,
                deferred_inst->threadNumber);
    }
}
...
WTB::doSquash(ThreadID tid)
{
    ...
    for(auto it_ld = ld_had_rbk.begin(); it_ld != ld_had_rbk.end() ; it_ld++){
            if((*it_ld)->seqNum <= squashedSeqNum[tid]){
                st_request.push_back((*it_ld));
            }
    }
    ld_had_rbk.clear();
    for(auto itst2 = st_request.begin(); itst2 != st_request.end() ; itst2++){
            ld_had_rbk.push_back((*itst2));
    }
    st_request.clear();
...
void
WTB::addIfReady(DynInstPtr &inst)
{
    inst->intoReadyInstsTick = curTick();
    // If the instruction now has all of its source registers
    // available, then add it to the list of ready instructions.
    if (inst->readyToIssue() && inst->inst_had_inReady == false) {
        ...
        if (inst->isMemRef()) {
            ...
            // its registers ready.
            if(inst->isStore() && inst->staInReadyList == false){
                inst->staInReadyList = true;
        ...
        if (!cpu->vsetBranch) {
            ...
            if (inst->isVsetvli() && !cpu->vecCsr.isReady(Vl, inst->prev_vl) &&
                inst->srcRegIdx(0).isZeroReg() && inst->destRegIdx(0).isZeroReg()) {
                    DPRINTF(RxuWTB, "[sn:%lli] has not ready out of veccsr [vl] sources.\n",
                        inst->seqNum);
                    if (!inst->hasAddToVlDependGraph) {
                        cpu->vecCsr.addConsumer(Vl, inst->prev_vl, inst);
                        inst->hasAddToVlDependGraph = true;
                    } 
                    return;                
            }

            if (inst->isVsetvli() && !cpu->vecCsr.isReady(Vl, inst->rn_vl) &&
                !inst->hasReadyVl) {
                wakeUpVsetvli *wakeUpEarly1 = new wakeUpVsetvli(this, inst);
                cpu->schedule(wakeUpEarly1, cpu->clockEdge(Cycles(1)));
                inst->hasReadyVl = true;
                stats.earlywakeupvsetvli++;
                DPRINTF(RxuWTB, "add if ready wake vl.\n");
            }
        }
        ...
        if (inst->ibuffer_id == 22 || inst->ibuffer_id == 23) {
            inst->inst_had_inReady = true;  
            /** Vector Special uop send issued info to macroop. */
            if (inst->isSpecialVector()) {
                inst->ori_inst->specialUopRdyNum++;
                DPRINTF(RxuWTB, "Special vector inst [sn:%llu] ready and specialUopRdyNum++.\n", inst->seqNum);
                if (!inst->ori_inst->isSpecialVectorNotExec()) {
...
WTB::arbiterIntSpecialAndBranch()
{
    ...
    DynInstPtr oddBrInst = nullptr;
    DynInstPtr oddBrInst1 = nullptr;
    DynInstPtr oddSpecialInst = nullptr;
    DynInstPtr evenJumpInst = nullptr;
    DynInstPtr evenBrInst = nullptr;
    DynInstPtr evenBrInst1 = nullptr;
    DynInstPtr evenSpecialInst = nullptr;
    ...
    if (!readyInsts[20][0].empty() && !oddJump) {
        oddBrInst1 = readyInsts[20][0].top();
        readyInsts[20][0].pop();
        ibuffer[oddBrInst1->threadNumber][20].remove(oddBrInst1);
        oddBr1 = true;
        allTempQ.push(oddBrInst1);
    }
    if (!readyInsts[21][0].empty() && !evenJump) {
        evenBrInst1 = readyInsts[21][0].top();
        readyInsts[21][0].pop();
        ibuffer[evenBrInst1->threadNumber][21].remove(evenBrInst1);
        evenBr1 = true;
        allTempQ.push(evenBrInst1);
    }
...
WTB::selectLdSt_dst()
{
    ...
    if(instold0){
        cnt_issue++;
        if(instold0->isLoad()){
            if(instold0->isOrderInst()){
                if(instold0 != dispipe3Stage->ldstOrderMap[instold0->ori_inst->ori_inst->seqNum].front()){
                    readyLD_Dst_odd.push(instold0);
                    DPRINTF(RxuWTB, "Load Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold0\n",instold0->seqNum);
                }
                else{
                    instold0->ori_inst->ori_inst->EopNotIssue--;
                    dispipe3Stage->ldstOrderMap[instold0->ori_inst->ori_inst->seqNum].pop();
                    ibufferToEW.push(instold0);
                    instold0->staIssued = true;
                    DPRINTF(RxuWTB, "Instruction Order Eop add to ibuffertoew odd0_1, [sn:%llu], ibuffer_id = %i.\n",instold0->seqNum, instold0->ibuffer_id);
                    if(instold0->ori_inst->ori_inst->EopNotIssue == 0){
                        dispipe3Stage->ldstOrderMap.erase(instold0->ori_inst->ori_inst->seqNum);
                        DPRINTF(RxuWTB, "Load Order Macro instruction [sn:%llu] all Eop had add to ibuffertoew\n", instold0->ori_inst->ori_inst->seqNum);
                    }
            ...
            if(instold0->isOrderInst()){
                if(instold0 != dispipe3Stage->ldstOrderMap[instold0->ori_inst->ori_inst->seqNum].front()){
                    readyStore.push(instold0);
                    DPRINTF(RxuWTB, "Store Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold0\n",instold0->seqNum);
                    ...
                    if(instold0->ori_inst->ori_inst->EopNotIssue == 0){
                        dispipe3Stage->ldstOrderMap.erase(instold0->ori_inst->ori_inst->seqNum);
                        DPRINTF(RxuWTB, "Store Order Macro instruction [sn:%llu] all Eop had add to ibuffertoew\n", instold0->ori_inst->ori_inst->seqNum);
                    }
                    ibuffer[instold0->threadNumber][instold0->ibuffer_id].remove(instold0);
                }
            }
    ...
    if(instold1){
        cnt_issue++;
        if(instold1->isLoad()){
            if(instold1->isOrderInst()){
                if(instold1 != dispipe3Stage->ldstOrderMap[instold1->ori_inst->ori_inst->seqNum].front()){
                    readyLD_Dst_odd.push(instold1);
                    DPRINTF(RxuWTB, "Load Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold1\n",instold1->seqNum);
                    
                }
                else{
                    instold1->ori_inst->ori_inst->EopNotIssue--;
                    dispipe3Stage->ldstOrderMap[instold1->ori_inst->ori_inst->seqNum].pop();
                    ibufferToEW.push(instold1);
                    instold1->staIssued = true;
                    DPRINTF(RxuWTB, "Instruction Order Eop add to ibuffertoew odd0_1, [sn:%llu], ibuffer_id = %i.\n",instold1->seqNum, instold1->ibuffer_id);
                    if(instold1->ori_inst->ori_inst->EopNotIssue == 0){
                        dispipe3Stage->ldstOrderMap.erase(instold1->ori_inst->ori_inst->seqNum);
                        DPRINTF(RxuWTB, "Load Order Macro instruction [sn:%llu] all Eop had add to ibuffertoew\n", instold1->ori_inst->ori_inst->seqNum);
                    }
            ...
            if(instold1->isOrderInst()){
                if(instold1 != dispipe3Stage->ldstOrderMap[instold1->ori_inst->ori_inst->seqNum].front()){
                    readyStore.push(instold1);
                    DPRINTF(RxuWTB, "Store Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold1\n",instold1->seqNum);
    ...
    if(instold2){
        cnt_issue++;
        if(instold2->isLoad()){
            if(instold2->isOrderInst()){
                if(instold2 != dispipe3Stage->ldstOrderMap[instold2->ori_inst->ori_inst->seqNum].front()){
                    readyLD_Dst_odd.push(instold2);
                    DPRINTF(RxuWTB, "Load Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold2\n",instold2->seqNum);
                    
                }
                else{
                    instold2->ori_inst->ori_inst->EopNotIssue--;
                    dispipe3Stage->ldstOrderMap[instold2->ori_inst->ori_inst->seqNum].pop();
                    ibufferToEW.push(instold2);
                    instold2->staIssued = true;
                    DPRINTF(RxuWTB, "Instruction Order Eop add to ibuffertoew odd0_1, [sn:%llu], ibuffer_id = %i.\n",instold2->seqNum, instold2->ibuffer_id);
                    if(instold2->ori_inst->ori_inst->EopNotIssue == 0){
                        dispipe3Stage->ldstOrderMap.erase(instold2->ori_inst->ori_inst->seqNum);
                        DPRINTF(RxuWTB, "Load Order Macro instruction [sn:%llu] all Eop had add to ibuffertoew\n", instold2->ori_inst->ori_inst->seqNum);
                    }
            ...
            if(instold2->isOrderInst()){
                if(instold2 != dispipe3Stage->ldstOrderMap[instold2->ori_inst->ori_inst->seqNum].front()){
                    readyStore.push(instold2);
                    DPRINTF(RxuWTB, "Store Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold2\n",instold2->seqNum);
    ...
    if(instold3){
        cnt_issue++;
        if(instold3->isLoad()){
            if(instold3->isOrderInst()){
                if(instold3 != dispipe3Stage->ldstOrderMap[instold3->ori_inst->ori_inst->seqNum].front()){
                    readyLD_Dst_odd.push(instold3);
                    DPRINTF(RxuWTB, "Load Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold3\n",instold3->seqNum);
                    
                }
                else{
                    instold3->ori_inst->ori_inst->EopNotIssue--;
                    dispipe3Stage->ldstOrderMap[instold3->ori_inst->ori_inst->seqNum].pop();
                    ibufferToEW.push(instold3);
                    instold3->staIssued = true;
                    DPRINTF(RxuWTB, "Instruction Order Eop add to ibuffertoew odd0_1, [sn:%llu], ibuffer_id = %i.\n",instold3->seqNum, instold3->ibuffer_id);
                    if(instold3->ori_inst->ori_inst->EopNotIssue == 0){
                        dispipe3Stage->ldstOrderMap.erase(instold3->ori_inst->ori_inst->seqNum);
                        DPRINTF(RxuWTB, "Load Order Macro instruction [sn:%llu] all Eop had add to ibuffertoew\n", instold3->ori_inst->ori_inst->seqNum);
                    }
            ...
            if(instold3->isOrderInst()){
                if(instold3 != dispipe3Stage->ldstOrderMap[instold3->ori_inst->ori_inst->seqNum].front()){
                    readyStore.push(instold3);
                    DPRINTF(RxuWTB, "Store Order Eop instruction [sn:%llu] can't issue due to not at orderqueue.head,instold3\n",instold3->seqNum);
...
WTB::setFreeReadRegPorts()
{
    ...
    ldstOddRegReadFreeNums  = ldstOddRegReadNums;
    ldstEvenRegReadFreeNums  = ldstEvenRegReadNums;
    ...
    vectorReadRegFreeNums = vectorReadRegNums;
    vectorLDSTReadRegFreeNums = vectorLDSTReadRegNums;
...
WTB::updateWbNums()
{
    ...
    newWbNums.intNormal_newEvenWbNums = 1;
    newWbNums.intNormal_newOddWbNums = 1;
...
{   
    if (!cpu->vsetBranch) {
        if (inst->isVsetvli() && !cpu->vecCsr.isReady(Vl, inst->rn_vl) &&
            !inst->hasReadyVl) {
            wakeUpVsetvli *wakeUpEarly2 = new wakeUpVsetvli(this, inst);
            cpu->schedule(wakeUpEarly2, cpu->clockEdge(Cycles(1)));
            stats.earlywakeupvsetvli++;
            DPRINTF(RxuWTB, "issue wake vl.\n");
        }
    }
...
WTB::processreadySrc_readReg(DynInstPtr &inst)
{
    ...
    // }
    if(new_inst->stdDataReady && new_inst->stdReady == false && new_inst->isStore()){
    ...
    if(new_inst->isStore() && new_inst->staInReadyList == false ){
        memDepUnit[new_inst->threadNumber].insert(new_inst);
...
WTB::processLdstWakeToIssue(DynInstPtr &inst)
{
    ...
    DPRINTF(RxuWTB, "Load/store instruction issued\n");
    DPRINTF(RxuWTB, "Instruction [sn:%i] is: %s.\n", inst->seqNum,
            inst->staticInst->disassemble(inst->pc->instAddr()));

    
    if (inst->lastWakeDependents == -1) {
...
void
WTB::processWakeUpVsetvli(DynInstPtr &inst)
{
    ...
    if (inst_name == "vsetvli" || inst_name == "vsetvl") {
        ...
        DPRINTF(RxuWTB, "Writing MiscReg vl (%d): %#x.\n", inst->rn_vl, newVlVal);               
    }

    cpu->vecCsr.setReady(Vl, inst->rn_vl);
    DPRINTF(RxuWTB, "waking up vl (%d): ready.\n", inst->rn_vl);
