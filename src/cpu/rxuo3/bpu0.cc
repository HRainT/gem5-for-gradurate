    ...
    if (!waitSendInsts[tid].empty()) {
        ...
        while (!insts_to_bpu1.empty() && toBpu1Index < bpu0Width) {
            ...
            if (bpu1_used && (inst->readPredTaken() != inst->readPredTaken0() || inst->readPredTarg().instAddr() != inst->readPredTarg0().instAddr())) {
                ++stats.correctByBpu;
                DPRINTF(RxuBpu0, "Done processing, taken branch instruction detected with PC = (%#x) by BPU.\n", inst->pcState().instAddr());
        ...
        for (int i = 0; i < fcl_to_bpu1.size; i++) {
            ...
            inst->seqNum = cpu->getAndIncrementInstSeq(incrementVector, incrementSplitMacro);
            incrementVector = false;
            incrementSplitMacro = false;


            if (inst->isMacroVector()) incrementVector = true;
            if (inst->isSplitMacro()) incrementSplitMacro = true;
            // Add instruction to the CPU's list of instructions.
            inst->setInstListIt(cpu->addInst(inst));
        ...
        const PCStateBase &next_pc, bool trace)
{
    ...
    InstSeqNum seq = cpu->getAndIncrementInstSeq(incrementVector,staticInst->isSplitMacro());
    incrementVector = false;
    ...
    if (instruction->isMacroVector()) incrementVector = true;
