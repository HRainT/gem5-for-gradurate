...
MemDepUnit::memhashadd(const DynInstPtr &inst){
    if(inst->memhash == false){
        ...
        DPRINTF(RxuMemDepUnit, "[sn:%llu] Inserted mem_hash\n",inst->seqNum);
    #ifdef GEM5_DEBUG
...
MemDepUnit::regsReady(const DynInstPtr &inst)
{
    ...
    if (inst_entry->memDeps == 0) {
        ...
        moveToReady(inst_entry);
        // if(inst_entry->inst->isStore() && inst_entry->inst->stdDataReady){
...
MemDepUnit::completed(const DynInstPtr &inst)
{
    DPRINTF(RxuMemDepUnit, "Completed mem instruction PC %s [sn:%lli].\n",
            inst->pcState(), inst->seqNum);
    if(inst->isVIndexORStrideLoad()){
        if(inst->ori_inst->isSquashed() || inst->isSquashed()){
            return;
        }
    }
...
MemDepUnit::squash(const InstSeqNum &squashed_num, ThreadID tid)
{
    ...
    for (auto squash_it = instList[tid].begin(); squash_it != instList[tid].end();) {
        if((*squash_it)->seqNum > squashed_num){
            ...
            memDepHash.erase(hash_it);
            squash_it = instList[tid].erase(squash_it);
        }
        else {
            ++squash_it;
        }
