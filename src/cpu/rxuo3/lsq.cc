...
LSQ::LSQStats::LSQStats(CPU *cpu)
    : statistics::Group(cpu, "lsq"),
      ADD_STAT(instToTlb_num, statistics::units::Count::get(),
               "Number of inst that went to TLB"),
      ADD_STAT(instToTlb_cycle, statistics::units::Count::get(),
               "Total cycles for inst to TLB"),
      ADD_STAT(ZeroCycle_num, statistics::units::Count::get(),
               "Total num for zero cycle"),
      ADD_STAT(instToTlb, "Distribution of cycle latency between the "
        ...
        AtomicOpFunctorPtr amo_op, const std::vector<bool>& byte_enable)
{
    ...
    if (request->isTranslationComplete()) {
        if(request->_tick && isLoad && !request->hadrecord){
            request->hadrecord = true;
            stats.instToTlb_num++;
            stats.instToTlb.sample(cpu->ticksToCycles(curTick() - request->_tick));
            Tick latency = curTick() - request->_tick;
            stats.instToTlb_cycle += latency;
            if(latency == 0){
                stats.ZeroCycle_num++;
            }
        }
        ...
        if (request->isMemAccessRequired()) {
            ...
            Fault fault;
            if(isLoad){
                Addr inst_eff_addr1 = inst->effAddr >> 4;
                Addr inst_eff_addr2 = (inst->effAddr + inst->effSize - 1) >> 4;
                for (auto it = cpu->dispipe3.wtb.memDepUnit[tid].stq.begin();
                    it != cpu->dispipe3.wtb.memDepUnit[tid].stq.end(); ++it) {
                        ...
                        Addr  store_eff_addr1 = it->second->effAddr >>4;
                        Addr  store_eff_addr2 = (it->second->effAddr + it->second->effSize - 1)  >>4;
