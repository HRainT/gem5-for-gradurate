...
IEW::IEW(CPU *_cpu, const BaseRxuO3CPUParams &params)
    : issueToExecQueue(params.backComSize, params.forwardComSize),
      ...
      dispipe2ToIEWDelay(params.dispipe2ToIEWDelay),
      ...
      iewStats(cpu)
{
    ...
    skidBufferMax = (dispipe2ToIEWDelay + 1) * params.dispipe2Width;
}
...
IEW::startupStage()
{
    for (ThreadID tid = 0; tid < numThreads; tid++) {
        toDispipe2->iewInfo[tid].usedIQ = true;
        toDispipe2->iewInfo[tid].freeIQEntries =
            instQueue.numFreeEntries(tid);

        toDispipe2->iewInfo[tid].usedLSQ = true;
        toDispipe2->iewInfo[tid].freeLQEntries =
            ldstQueue.numFreeLoadEntries(tid);
        toDispipe2->iewInfo[tid].freeSQEntries =
...
IEW::clearStates(ThreadID tid)
{
    ...
    toDispipe2->iewInfo[tid].usedLSQ = true;
    toDispipe2->iewInfo[tid].freeLQEntries = ldstQueue.numFreeLoadEntries(tid);
    toDispipe2->iewInfo[tid].freeSQEntries = ldstQueue.numFreeStoreEntries(tid);
}
...
void
IEW::setDispipe2Queue(TimeBuffer<Dispipe2Struct> *p2q_ptr)
{
    dispipe2Queue = p2q_ptr;

    // Setup wire to read information from dispipe2 queue.
    fromDispipe2 = dispipe2Queue->getWire(-dispipe2ToIEWDelay);
}
...
{
    DPRINTF(RxuIEW, "[tid:%i] Squashing all instructions.\n", tid);
    ...
    while (!skidBuffer[tid].empty()) {
        if (skidBuffer[tid].front()->isLoad()) {
            toDispipe2->iewInfo[tid].dispatchedToLQ++;
        }
        if (skidBuffer[tid].front()->isStore() ||
            skidBuffer[tid].front()->isAtomic()) {
            toDispipe2->iewInfo[tid].dispatchedToSQ++;
        }

        toDispipe2->iewInfo[tid].dispatched++;

        skidBuffer[tid].pop();
    }

    emptyDispipe2Insts(tid);
}

void
IEW::squashDueToBranch(const DynInstPtr& inst, ThreadID tid)
{
    DPRINTF(RxuIEW, "[tid:%i] [sn:%llu] Squashing from a specific instruction,"
            " PC: %s "
...
{
    DPRINTF(RxuIEW, "[tid:%i] Memory violation, squashing violator and younger "
...
{
    DPRINTF(RxuIEW, "[tid:%i] Blocking.\n", tid);

    if (dispatchStatus[tid] != Blocked &&
        dispatchStatus[tid] != Unblocking) {
        toDispipe2->iewBlock[tid] = true;
        wroteToTimeBuffer = true;
    }
...
{
    DPRINTF(RxuIEW, "[tid:%i] Reading instructions out of the skid "
            "buffer %u.\n",tid, tid);

    // If the skid bufffer is empty, signal back to previous stages to unblock.
    // Also switch status to running.
    if (skidBuffer[tid].empty()) {
        toDispipe2->iewUnblock[tid] = true;
        wroteToTimeBuffer = true;
        DPRINTF(RxuIEW, "[tid:%i] Done unblocking.\n",tid);
        dispatchStatus[tid] = Running;
    }
}
...
IEW::instToCommit(const DynInstPtr& inst)
{
    ...
    DPRINTF(RxuIEW, "Current wb cycle: %i, width: %i, numInst: %i\nwbActual:%i\n",
...
IEW::skidInsert(ThreadID tid)
{
    ...
    while (!insts[tid].empty()) {
        ...
        DPRINTF(RxuIEW,"[tid:%i] Inserting [sn:%lli] PC:%s into "
...
IEW::updateStatus()
{
    ...
    if (_status == Active && !instQueue.hasReadyInsts() &&
        ...
        DPRINTF(RxuIEW, "IEW switching to idle\n");
        ...
        DPRINTF(RxuIEW, "IEW switching to active\n");
...
IEW::checkStall(ThreadID tid)
{
    ...
    if (fromCommit->commitInfo[tid].robSquashing) {
        DPRINTF(RxuIEW,"[tid:%i] Stall from Commit stage detected.\n",tid);
        ret_val = true;
    } else if (instQueue.isFull(tid)) {
        DPRINTF(RxuIEW,"[tid:%i] Stall: IQ  is full.\n",tid);
        ret_val = true;
    }
...
IEW::checkSignalsAndUpdate(ThreadID tid)
{
    ...
    if (fromCommit->commitInfo[tid].robSquashing) {
        DPRINTF(RxuIEW, "[tid:%i] ROB is still squashing.\n", tid);
    ...
    if (dispatchStatus[tid] == Blocked) {
        ...
        DPRINTF(RxuIEW, "[tid:%i] Done blocking, switching to unblocking.\n",
                tid);
    ...
    if (dispatchStatus[tid] == Squashing) {
        // Switch status to running if dispipe2 isn't being told to block or
        // squash this cycle.
        DPRINTF(RxuIEW, "[tid:%i] Done squashing, switching to running.\n",
                tid);
...
IEW::sortInsts()
{
    ...
    for (int i = 0; i < insts_from_dispipe2; ++i) {
        insts[fromDispipe2->insts[i]->threadNumber].push(fromDispipe2->insts[i]);
    }
}

void
IEW::emptyDispipe2Insts(ThreadID tid)
{
    DPRINTF(RxuIEW, "[tid:%i] Removing incoming dispipe2 instructions\n", tid);
...
IEW::dispatch(ThreadID tid)
{
    ...
    //     buffer any instructions coming from dispipe2
    ...
    if (dispatchStatus[tid] == Running ||
        ...
        DPRINTF(RxuIEW, "[tid:%i] Not blocked, so attempting to run "
                "dispatch.\n", tid);
...
{
    // Obtain instructions from skid buffer if unblocking, or queue from dispipe2
    // otherwise.
          ...
          ++dis_num_inst)
    {
        ...
        if (dispatchStatus[tid] == Unblocking) {
            DPRINTF(RxuIEW, "[tid:%i] Issue: Examining instruction from skid "
                    "buffer\n", tid);
        }
        ...
        DPRINTF(RxuIEW, "[tid:%i] Issue: Adding PC %s [sn:%lli] [tid:%i] to "
                "IQ.\n",
        ...
        if (inst->isSquashed()) {
            DPRINTF(RxuIEW, "[tid:%i] Issue: Squashed instruction encountered, "
                    "not adding to IQ.\n", tid);
            ...
            //Tell Dispipe2 That An Instruction has been processed
            if (inst->isLoad()) {
                toDispipe2->iewInfo[tid].dispatchedToLQ++;
            }
        ...
        if (instQueue.isFull(tid)) {
            DPRINTF(RxuIEW, "[tid:%i] Issue: IQ has become full.\n", tid);
            ...
            // get full in the IQ.
            toDispipe2->iewUnblock[tid] = false;
        ...
        if ((inst->isAtomic() && ldstQueue.sqFull(tid)) ||
            ...
            DPRINTF(RxuIEW, "[tid:%i] Issue: %s has become full.\n",tid,
                    inst->isLoad() ? "LQ" : "SQ");
        ...
        if (inst->isAtomic()) {
            DPRINTF(RxuIEW, "[tid:%i] Issue: Memory instruction "
            ...
            DPRINTF(RxuIEW, "[tid:%i] Issue: Nop instruction encountered, "
                    "skipping.\n", tid);
        ...
        if (add_to_iq && inst->isNonSpeculative()) {
            DPRINTF(RxuIEW, "[tid:%i] Issue: Nonspeculative instruction "
    ...
    if (!insts_to_dispatch.empty()) {
        DPRINTF(RxuIEW,"[tid:%i] Issue: Bandwidth Full. Blocking.\n", tid);
        block(tid);
        toDispipe2->iewUnblock[tid] = false;
    }
...
//    printAvailableInsts();
    ...
    for (; inst_num < insts_to_execute;
        ...
        DPRINTF(RxuIEW, "Execute: Executing instructions from IQ.\n");

        DynInstPtr inst = instQueue.getInstToExecute();

        DPRINTF(RxuIEW, "Execute: Processing PC %s, [tid:%i] [sn:%llu].\n",
        ...
        if (inst->isSquashed()) {
            DPRINTF(RxuIEW, "Execute: Instruction was squashed. PC: %s, [tid:%i]"
        ...
        // at the commit stage.
        if (inst->isMemRef()) {
            DPRINTF(RxuIEW, "Execute: Calculating address for memory "
                    "reference.\n");
            ...
            if (inst->isAtomic()) {
                ...
                if (inst->isTranslationDelayed() &&
                    ...
                    DPRINTF(RxuIEW, "Execute: Delayed translation, deferring "
                            "store.\n");
        ...
        if (!fetchRedirect[tid] ||
            ...
            if (inst->mispredicted() && !loadNotExecuted) {
                ...
                DPRINTF(RxuIEW, "[tid:%i] [sn:%llu] Execute: "
                        "Branch mispredict detected.\n",
                        tid, inst->seqNum);
                DPRINTF(RxuIEW, "[tid:%i] [sn:%llu] "
                ...
                DPRINTF(RxuIEW, "LDSTQ detected a violation. Violator PC: %s "
            ...
            if (ldstQueue.violation(tid)) {
                ...
                DPRINTF(RxuIEW, "LDSTQ detected a violation.  Violator PC: "
                        "%s, inst PC: %s.  Addr is: %#x.\n",
                        violator->pcState(), inst->pcState(),
                        inst->physEffAddr);
                DPRINTF(RxuIEW, "Violation will not be handled because "
                        "already squashing\n");
...
IEW::writebackInsts()
{
    ...
    for (int inst_num = 0; inst_num < wbWidth &&
        ...
        DPRINTF(RxuIEW, "Sending instructions to commit, [sn:%lli] PC %s.\n",
        ...
        if (!inst->isSquashed() && inst->isExecuted() &&
            ...
            for (int i = 0; i < inst->numDestRegs(); i++) {
                ...
                if (inst->renamedDestIdx(i)->
                    ...
                    DPRINTF(RxuIEW,"Setting Destination Register %i (%s)\n",
...
IEW::tick()
{
    ...
    while (threads != end) {
        ...
        DPRINTF(RxuIEW,"Issue: Processing [tid:%i]\n",tid);
    ...
    while (threads != end) {
        ...
        DPRINTF(RxuIEW,"Processing [tid:%i]\n",tid);
        ...
        if (fromCommit->commitInfo[tid].nonSpecSeqNum != 0) {
            ...
            //DPRINTF(RxuIEW,"NonspecInst from thread %i",tid);
        ...
        DPRINTF(RxuIEW, "[tid:%i], Dispatch dispatched %i instructions.\n",
                tid, toDispipe2->iewInfo[tid].dispatched);
    }

    DPRINTF(RxuIEW, "IQ has %i free entries (Can schedule: %i).  "
    ...
    if (!fetchRedirect[tid] ||
        ...
        if (inst->mispredicted()) {
            ...
            DPRINTF(RxuIEW, "[tid:%i] [sn:%llu] Execute: "
                    "Branch mispredict detected.\n",
                    tid, inst->seqNum);
            DPRINTF(RxuIEW, "[tid:%i] [sn:%llu] Predicted target was PC: %s\n",
                    tid, inst->seqNum, inst->readPredTarg());
            DPRINTF(RxuIEW, "[tid:%i] [sn:%llu] Execute: "
                    "Redirecting fetch to PC: %s\n",
                    tid, inst->seqNum, inst->pcState());
