...
RenameUnit::renameDestRegs(const DynInstPtr &inst, ThreadID tid, unsigned inst_index)
{
    ...
    if (!cpu->vsetBranch) {
        if (inst->isVset()) {
            ...
            if (inst->isVsetivli() || inst->isVsetvli() && !inst->destRegIdx(0).isZeroReg() && inst->srcRegIdx(0).isZeroReg()) {
                writeImmVl(inst, tid);
            }
        }
...
RenameUnit::renameSrcRegs(const DynInstPtr &inst, ThreadID tid)
{
    ...
    if (!cpu->vsetBranch) {
        if (inst->isVlff() && inst->isLastMicroop()) {
            ...
            DPRINTF(RxuRename,
                    "[tid:%i] "
                    "Vlff renaming dest: arch reg vl to physical reg %i. "
                    "Old mapping is %i.\n",
                    tid, new_rn_vl, prev_rn_vl);
    ...
    for (int src_idx = 0; src_idx < num_src_regs; src_idx++)
    {
        ...
        {   
            if (1 && inst->isMicroVector() && num_src_regs >= 2 && inst->numDestRegs()
                 && !inst->ori_inst->isSpecialVectorNotExec() && !inst->isRxuVmInst() && inst->if_ta_ma) {
...
RenameUnit::getVlVtype(const DynInstPtr &inst)
{
    ...
    DPRINTF(RxuRename,
            "[sn:%lli]Get vl/vtype from physical regfile: vl=%d, vtype=0x%x.\n",
            inst->seqNum, inst->new_vl, vtype);
}
...
RenameUnit::microInheritMacroRename(const DynInstPtr &inst, int src_idx)
{
    if (inst->ori_inst->ifrenamesrc) {
        ...
        if (scoreboard->getReg(renamed_reg))
        {
            ...
            if(inst->isMemRef() && !inst->isVector()){
        ...
        else if (1 && inst->isEop() && num_src_regs >= 2 && !inst->ori_inst->isSpecialVectorNotExec() && !inst->isRxuVmInst() && inst->if_ta_ma) {
                if (inst->staticInst->machInst.vm && src_idx == (num_src_regs -1) && src_reg.index() == inst->ori_inst->destRegIdx(0).index()) {//TODO
                    inst->markSrcRegReady(src_idx);                
                ...
                } else if (!inst->staticInst->machInst.vm && src_idx == (num_src_regs -2) && src_reg.index() == inst->ori_inst->destRegIdx(0).index()) {
                    inst->markSrcRegReady(src_idx);                
...
RenameUnit::writeImmVl(const DynInstPtr &inst, ThreadID tid)
{
    ...
    if (!inst->destRegIdx(0).isZeroReg() && inst->srcRegIdx(0).isZeroReg()) {
        cpu->vecCsr.setVal(Vl, inst->rn_vl, vlmax);
