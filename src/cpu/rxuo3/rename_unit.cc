...
RenameUnit::renameDestRegs(const DynInstPtr &inst, ThreadID tid, unsigned inst_index)
{
    ...
    if (inst->isVecCsrVxrm()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Renaming arch reg vxrm to physical reg %i. "
                "Old mapping is %i.\n",
    ...
    if (inst->isVset()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Renaming arch reg vl to physical reg %i. "
                "Old mapping is %i.\n",
    ...
    if (inst->isVset()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Renaming arch reg vtype to physical reg %i. "
                "Old mapping is %i.\n",
...
RenameUnit::renameSrcRegs(const DynInstPtr &inst, ThreadID tid)
{
    ...
    if (inst->isMacroVector()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Looking up vtype, got phys reg %i.\n",
                tid, curVtypeIndex);
    ...
    if (inst->needVxrm() && inst->isMicroVector()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Looking up vxrm, got phys reg %i.\n",
                tid, curVxrmIndex);
    ...
    if (inst->isCsrVl()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Looking up csr vl, got phys reg %i.\n",
                tid, curVlIndex);
        if (vecCsr->isReady(Vl,curVlIndex)) {
            DPRINTF(RxuRename,
                    "[tid:%i] "
    ...
    if (inst->isCsrVtype()) {
        ...
        DPRINTF(RxuRename,
                "[tid:%i] "
                "Looking up csr vtype, got phys reg %i.\n",
                tid, curVtypeIndex);
        if (vecCsr->isReady(Vtype,curVtypeIndex)) {
            DPRINTF(RxuRename,
                    "[tid:%i] "
    ...
    for (int src_idx = 0; src_idx < num_src_regs; src_idx++)
    {
        ...
        // if (src_reg.regClass().type() == VecRegClass && inst->isMacroVector()) {
        //     DPRINTF(RxuRename, "MacroVector looking up %s arch reg %i and continue.\n", 
        //             src_reg.regClass().type(), src_reg.index());
        //     continue; 
        // }
        ...
        if (inst->isMacroop()) {
            DPRINTF(RxuRename,
                    "[tid:%i] "
                    "MacroVector looking up %s arch reg %i, got phys reg %i (%s)\n",
...
RenameUnit::microInheritMacroRename(const DynInstPtr &inst, int src_idx)
{
    if (inst->ori_inst->ifrenamesrc) {
        ...
        PhysRegIdPtr copied_reg = new PhysRegId(*renamed_reg);
        inst->renameSrcReg(src_idx, copied_reg);
        inst->ifrenamesrc = true;
        // See if the register is ready or not.
        if (scoreboard->getReg(copied_reg))
        {
            DPRINTF(RxuRename,
                    "Inherit register %d (flat: %d) (%s) is ready.\n",
                    copied_reg->index(), copied_reg->flatIndex(),
                    copied_reg->className());
...
RenameUnit::writeImmVl(const DynInstPtr &inst, ThreadID tid)
{
    ...
    RegVal newVlVal = 0;
    RegVal oldVlVal = cpu->vecCsr.getVal(Vl, inst->rn_vl);
...
{   
    ...
    firstAndLast = inst->isLastMicroop() && inst->isFirstMicroop();
    ...
    if (firstAndLast) {
        it = historyBuffer[tid].erase(it);
        historyBuffer[tid].insert(it, hb_entry);
    } else if (inst->isFirstMicroop()) {
        micro_it = (--it);
        historyBuffer[tid].insert(micro_it, hb_entry);
        micro_it--;
    } else if (inst->isLastMicroop()) {
        it = historyBuffer[tid].erase(it);
        historyBuffer[tid].insert(micro_it, hb_entry);
    } else {
        historyBuffer[tid].insert(micro_it, hb_entry);
