#ifndef __CPU_RxuO3_MEM_DEP_UNIT_HH__
#define __CPU_RxuO3_MEM_DEP_UNIT_HH__
 ...
 * As memory operations are issued to the WTB, they are also issued to this
...
class MemDepUnit
{
  ...
  public:
    ...
    void validlfst(const DynInstPtr &inst);

    void memhashadd(const DynInstPtr &inst);

    /** The memory dependence predictor.  It is accessed upon new
     *  instructions being added to the WTB, and responds by telling
     *  this unit what instruction the newly added instruction is dependent
     *  upon.
     */
    StoreSet depPred;

    std::map<InstSeqNum, DynInstPtr> stq;
    std::map<InstSeqNum, DynInstPtr> ldq;
    std::map<InstSeqNum, std::list<InstSeqNum>> ldstdep;
    ...
    MemDepUnit(const BaseRxuO3CPUParams &params);
    ...
    void init(const BaseRxuO3CPUParams &params, ThreadID tid, CPU *cpu);
    ...
    /** Sets the pointer to the WTB. */
    void setWTB(WTB *wtb_ptr);

    /** Inserts a memory instruction. */
    void insert(const DynInstPtr &inst);

    void insert_test(const DynInstPtr &inst);
  ...
  private:
    ...
    std::vector<MemDepEntryPtr> inst_aheadwtb;
...
#endif // __CPU_RxuO3_MEM_DEP_UNIT_HH__
