...
void
SimpleRenameMap::init(const RegClass &reg_class, RxuSimpleFreeList *_freeList)
{
...
{
    assert(o3freeList == NULL);
    assert(map.empty());

    map.resize(reg_class.numRegs());
    o3freeList = _freeList;
}
...
SimpleRenameMap::RenameInfo
SimpleRenameMap::rename(const RegId& arch_reg, unsigned inst_index, bool& stall)
// SimpleRenameMap::rename(const RegId& arch_reg)
{
    PhysRegIdPtr renamed_reg;
    // Record the current physical register that is renamed to the
    ...
    if (arch_reg.is(InvalidRegClass)) {
        ...
        renamed_reg = freeList->getReg(arch_reg, inst_index, stall);
        if (stall || !renamed_reg) {
            return RenameInfo(nullptr, prev_reg);
        }
        // ------------------------------------------
