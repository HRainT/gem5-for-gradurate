#ifndef __CPU_RxuO3_LSQ_UNIT_HH__
#define __CPU_RxuO3_LSQ_UNIT_HH__
...
uint16_t make_int_digest(uint64_t value, int reg_num) {
  ...
  public:
    ...
    void init(CPU *cpu_ptr, EW *ew_ptr, Dispipe3 *p3_ptr, const BaseRxuO3CPUParams &params,
    ...
    unsigned getCountstore() { return storeQueue.size(); }

    unsigned getCountload() { return loadQueue.size(); }
  ...
  private:
    ...
    /** Writes back the instruction, sending it to EW. */
  ...
  private:
    ...
    /** Pointer to the EW stage. */
    EW *ewStage;

    /** Pointer to the Dispipe3 stage. */
  ...
  protected:
    ...
    struct LSQUnitStats : public statistics::Group
    {
        ...
        /** Number of times the LSQ is blocked due to the cachecport. */
        statistics::Scalar CacheNoPort;
        ...
        // eop cross cacheline stats
        statistics::Scalar crossCacheline;
        statistics::Scalar stqsize;
    } stats;
...
#endif // __CPU_RxuO3_LSQ_UNIT_HH__
