      ...
      _readySrcIdx(arrays.readySrcIdx), macroop(_macroop)
{
    ...
    if (cpu->instcount > 500000000) {
#ifdef GEM5_DEBUG
        cpu->dumpInsts();
        dumpSNList();
#endif
        assert(cpu->instcount <= 500000000);
    }
...
DynInst::~DynInst()
{
    ...
    if (debug::RxuO3PipeView) {
        ...
        if (fetch != -1) {
            ...
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:fetch:%llu:0x%08llx:%d:%llu:%s\n",
                     fetch,
                     pcState().instAddr(),
                     pcState().microPC(),
                     seqNum,
                     staticInst->disassemble(pcState().instAddr()));
            val = (IBandLBTick == -1) ? 0 : fetch + IBandLBTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:IBandLB:%llu\n", val);
            val = (decodeTick == -1) ? 0 : fetch + decodeTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:decode:%llu\n", val);
            // -- add by hongfei.liu -------------------------------------
            val = (predisqTick == -1) ? 0 : fetch + predisqTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:predisq:%llu\n", val);
            // -----------------------------------------------------------
            val = (renameTick == -1) ? 0 : fetch + renameTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:rename:%llu\n", val);
            val = (dispatchTick == -1) ? 0 : fetch + dispatchTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:dispatch:%llu\n", val);
            val = (issueTick == -1) ? 0 : fetch + issueTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:issue:%llu\n", val);
            val = (completeTick == -1) ? 0 : fetch + completeTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:complete:%llu\n", val);
            val = (commitTick == -1) ? 0 : fetch + commitTick;

            Tick valS = (storeTick == -1) ? 0 : fetch + storeTick;
            DPRINTFR(RxuO3PipeView, "RxuO3PipeView:retire:%llu:store:%llu\n",
                    val, valS);
        }
    }
#endif
...
{
    DPRINTF(RxuWTB, "[sn:%lli] has %d ready out of %d sources. RTI %d)\n",
...
void
DynInst::markSrcRegReady(int src_idx,bool srcready,int srcx)
{
    // bool srcxready = readySrcIdx(src_idx);
    if (src_idx == 0  && srcx == 0) {
    readySrcIdx(src_idx, true);
    DPRINTF(RxuWTB, "[sn:%lli] has src0 ready out of %d sources. RTI %d)\n",
        seqNum, numSrcRegs(), readyToIssue());
        setCanIssue();
    }
    if(src_idx == 1  && srcx == 1){
    readySrcIdx(src_idx, true);
    DPRINTF(RxuWTB, "[sn:%lli] has src1 ready out of %d sources.)\n",
        seqNum, numSrcRegs()); 
        stdDataReady = true;
    } 
}
...
void
DynInst::markSrcRegReady(int src_idx,bool srcready,int srcx,bool is_store,bool is_index_stride)
{   
    src0_ready = readySrcIdx(0);
    src1_ready = readySrcIdx(1);
    src2_ready = readySrcIdx(2);
    src3_ready = readySrcIdx(3);
    if(is_store && isVector()){
        if(staticInst->machInst.vm){
            if(isVIndexORStrideStore() && !isVsseg()){
                if (src_idx == 0  && srcx == 0) {
                    readySrcIdx(src_idx, true);
                    src0_ready = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has src0 ready out of %d sources.)\n",
                    seqNum, numSrcRegs());
                }
                else if(src_idx == 1  && srcx == 1){
                    readySrcIdx(src_idx, true);
                    src1_ready = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has src1 ready out of %d sources.)\n",
                    seqNum, numSrcRegs());
                }
                else if(src_idx == 2  && srcx == 2){
                    readySrcIdx(src_idx, true);
                    src2_ready = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has src2 ready out of %d sources.)\n",
                    seqNum, numSrcRegs());
                }
                else {
                    panic("VIndexORStrideStore no vm over 3 registers");
                }
                if(src0_ready && src1_ready){
                    setCanIssue();
                    DPRINTF(RxuWTB, "[sn:%lli] has all of sta ready,canIssue\n",seqNum);
                }
                if(src2_ready){
                    stdDataReady = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has all of std ready,std can arb\n",seqNum);
                }
            }
            else if(isVse() || isVsseg()){
                if (src_idx == 0  && srcx == 0) {
                    readySrcIdx(src_idx, true);
                    src0_ready = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has src0 ready out of %d sources.)\n",
                    ...
                    panic("vsseg or vse no vm over 2 src registers");
                }
                if(src0_ready){
                    setCanIssue();
                    DPRINTF(RxuWTB, "[sn:%lli] has all of sta ready,canIssue\n",seqNum);
            ...
            if(isVIndexORStrideStore()){
                ...
                else if(src_idx == 3  && srcx == 3){
                    readySrcIdx(src_idx, true);
                    src3_ready = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has src3 ready out of %d sources.)\n",
                    seqNum, numSrcRegs());
                }
                else {
                    panic("vst over 4 src registers");
                }
                if(src0_ready && src1_ready){
                    setCanIssue();
                    DPRINTF(RxuWTB, "[sn:%lli] has all of sta ready,canIssue\n",seqNum);
                }
                if(src2_ready && src3_ready){
                    stdDataReady = true;
                    DPRINTF(RxuWTB, "[sn:%lli] has all of std ready,std can arb\n",seqNum);
                }
            }
                ...
                if (src_idx == 0  && srcx == 0) {
                    ...
                    panic("vsseg vse over 3 src registers");
                }
                if(src0_ready){
                    setCanIssue();
                    DPRINTF(RxuWTB, "[sn:%lli] has all of sta ready,canIssue\n",seqNum);
        ...
        panic("inst is not vector but use vector wake");
    }
    else {
        readySrcIdx(src_idx, true);
            DPRINTF(RxuWTB, "[sn:%lli] has src%x ready out of %d sources.)\n",
            seqNum,srcx,numSrcRegs()); 
        ++readyRegs_vector;
        if(readyRegs_vector == numSrcRegs()){
            setCanIssue();
        }
    }
}
...
{   
    ...
    if (status[Squashed]) return;

    if (isMicroVector() && !isNonSplitVector()) {
        ori_inst->setSquashed();
        ori_inst->macroMemRefIsErased = false;
    }
    status.set(Squashed);
...
DynInst::execute()
{
    ...
    if (isMicroVector() || isEop()) setOriFault();
...
DynInst::printDisassemblyAndResult(const std::string &site) const
{
    ...
    if (instResult.size() > 0) {
        DPRINTFR(CommitTrace, ", res: %#lx", instResult.front().asRegVal());
    }
    if (isMemRef()) {
