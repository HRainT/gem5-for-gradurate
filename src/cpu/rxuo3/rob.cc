...
ROB::insertInst(const DynInstPtr &inst)
{
    ...
    DPRINTF(RxuROB, "[tid:%i] Now has %d instructions.\n", tid,
            threadEntries[tid]);
...
ROB::retireHead(ThreadID tid)
{
    ...
    if (head_inst->isLastMicroop() && head_inst->isMicroVector()) {
...
{    
    ...
    if (it != instList[tid].end() && !uopList.empty()) {
        ...
        if (macro_inst->isNonSplitVector()) {
            return;
        }
        // macro_inst->clearInROB();
        // macro_inst->setCommitted();

        // cpu->removeInstsThisCycle = true;
        // cpu->removeList.push(macro_inst->getInstListIt());
        // macro_inst->setInRemove();

        DPRINTF(RxuROB, "[tid:%i] Now has %d vector instructions.\n", tid,
                threadVecEntries[tid]);  
