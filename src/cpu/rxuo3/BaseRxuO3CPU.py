...
class BaseRxuO3CPU(BaseCPU):
    ...
    ldstRegReadNums = Param.Unsigned(4, "Number of load/store regfile read ports")
    ...
    vectorRegWriteNums = Param.Unsigned(9, "Number of float load regfile write ports")

    # other params
    backComSize = Param.Unsigned(
        10, "Time buffer size for backwards communication"
    )
    forwardComSize = Param.Unsigned(
        10, "Time buffer size for forward communication"
    )

    LQEntries = Param.Unsigned(164, "Number of load queue entries")
    SQEntries = Param.Unsigned(112, "Number of store queue entries")
