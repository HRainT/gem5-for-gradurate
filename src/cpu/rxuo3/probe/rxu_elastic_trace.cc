#include "cpu/rxuo3/probe/rxu_elastic_trace.hh"
...
#include "debug/RxuElasticTrace.hh"
...
RxuElasticTrace::RxuElasticTrace(const RxuElasticTraceParams &params)
       ...
       stats(this)
{
    ...
    fatal_if(!cpu, "Manager of %s is not of type RxuO3CPU and thus does not "\
                "support dependency tracing.\n", name());

    fatal_if(depWindowSize == 0, "depWindowSize parameter must be non-zero. "\
                "Recommended size is 3x ROB size in the RxuO3CPU.\n");
...
void
RxuElasticTrace::regProbeListeners()
{
...
void
RxuElasticTrace::regEtraceListeners()
{
    assert(!allProbesReg);
    ...
    // each probe point.
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace, RequestPtr>(this,
                        "FetchRequest", &RxuElasticTrace::fetchReqTrace));
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace,
            DynInstConstPtr>(this, "Execute",
                &RxuElasticTrace::recordExecTick));
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace,
            DynInstConstPtr>(this, "ToCommit",
                &RxuElasticTrace::recordToCommTick));
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace,
            DynInstConstPtr>(this, "Rename",
                &RxuElasticTrace::updateRegDep));
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace, SeqNumRegPair>(this,
                        "SquashInRename", &RxuElasticTrace::removeRegDepMapEntry));
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace,
            DynInstConstPtr>(this, "Squash",
                &RxuElasticTrace::addSquashedInst));
    listeners.push_back(new ProbeListenerArg<RxuElasticTrace,
            DynInstConstPtr>(this, "Commit",
                &RxuElasticTrace::addCommittedInst));
    allProbesReg = true;
}

void
RxuElasticTrace::fetchReqTrace(const RequestPtr &req)
{

    DPRINTFR(RxuElasticTrace, "Fetch Req %i,(%lli,%lli,%lli),%i,%i,%lli\n",
             (MemCmd::ReadReq),
...
void
RxuElasticTrace::recordExecTick(const DynInstConstPtr& dyn_inst)
{
    ...
    if (dyn_inst->seqNum <= lastClearedSeqNum) {
        DPRINTFR(RxuElasticTrace, "[sn:%lli] Ignoring in execute as instruction \
    ...
    DPRINTFR(RxuElasticTrace, "[sn:%lli] Execute Tick = %i\n", dyn_inst->seqNum,
                curTick());
...
void
RxuElasticTrace::recordToCommTick(const DynInstConstPtr& dyn_inst)
{
    ...
    auto itr_exec_info = tempStore.find(dyn_inst->seqNum);
    if (itr_exec_info == tempStore.end()) {
        DPRINTFR(RxuElasticTrace, "recordToCommTick: [sn:%lli] Not in temp store,"
                    " skipping.\n", dyn_inst->seqNum);
        return;
    }

    DPRINTFR(RxuElasticTrace, "[sn:%lli] To Commit Tick = %i\n", dyn_inst->seqNum,
                curTick());
...
void
RxuElasticTrace::updateRegDep(const DynInstConstPtr& dyn_inst)
{
    ...
    for (int src_idx = 0; src_idx < max_regs; src_idx++) {
        ...
        if (!src_reg.is(MiscRegClass) && !src_reg.is(InvalidRegClass)) {
            ...
            DPRINTFR(RxuElasticTrace, "[sn:%lli] Check map for src reg"
                     " %i (%s)\n", seq_num,
    ...
    for (int dest_idx = 0; dest_idx < max_regs; dest_idx++) {
        ...
        if (!dest_reg.is(MiscRegClass) && !dest_reg.is(InvalidRegClass)) {
            ...
            DPRINTFR(RxuElasticTrace, "[sn:%lli] Update map for dest reg"
...
void
RxuElasticTrace::removeRegDepMapEntry(const SeqNumRegPair &inst_reg_pair)
{
    DPRINTFR(RxuElasticTrace, "Remove Map entry for Reg %i\n",
...
void
RxuElasticTrace::addSquashedInst(const DynInstConstPtr& head_inst)
{
    ...
    DPRINTFR(RxuElasticTrace, "Attempt to add squashed inst [sn:%lli]\n",
                head_inst->seqNum);
...
void
RxuElasticTrace::addCommittedInst(const DynInstConstPtr& head_inst)
{
    DPRINTFR(RxuElasticTrace, "Attempt to add committed inst [sn:%lli]\n",
                head_inst->seqNum);
    ...
    if (!head_inst->isNop()) {
        ...
        if (itr_temp_store == tempStore.end()) {
            DPRINTFR(RxuElasticTrace, "addCommittedInst: [sn:%lli] Not in temp "
                "store, skipping.\n", head_inst->seqNum);
            return;
        }
        ...
        if (head_inst->getFault() != NoFault) {
            DPRINTF(RxuElasticTrace, "%s [sn:%lli] has faulted so "
                    "skip adding it to the trace\n",
                    (head_inst->isMemRef() ? "Load/store" : "Comp inst."),
                    head_inst->seqNum);
        } else if (head_inst->isMemRef() && !head_inst->hasRequest()) {
            DPRINTF(RxuElasticTrace, "Load/store [sn:%lli]  has no request so "
                    "skip adding it to the trace\n", head_inst->seqNum);
        } else if (!head_inst->readPredicate()) {
            DPRINTF(RxuElasticTrace, "%s [sn:%lli] is predicated false so "
                    "skip adding it to the trace\n",
                    (head_inst->isMemRef() ? "Load/store" : "Comp inst."),
                    head_inst->seqNum);
        } else {
...
void
RxuElasticTrace::addDepTraceRecord(const DynInstConstPtr& head_inst,
                                InstExecInfo* exec_info_ptr, bool commit)
{
    ...
    if (depTrace.empty()) {
        ...
        DPRINTF(RxuElasticTrace, "Added first inst record %lli to DepTrace.\n",
                new_record->instNum);
        return;
    }
    ...
    for (dep_set_it = (exec_info_ptr->physRegDepSet).begin();
        ...
        if (trace_info_itr != traceInfoMap.end()) {
            ...
            DPRINTF(RxuElasticTrace, "Inst %lli has register dependency on "
    ...
    // Store the record in depTrace.
    depTrace.push_back(new_record);
    DPRINTF(RxuElasticTrace, "Added %s inst %lli to DepTrace.\n",
    ...
    if (depTrace.size() == 2 * depWindowSize) {
        ...
        DPRINTF(RxuElasticTrace, "Writing out trace...\n");
...
void
RxuElasticTrace::updateCommitOrderDep(TraceInfo* new_record,
...
void
RxuElasticTrace::updateIssueOrderDep(TraceInfo* new_record)
{
...
void
RxuElasticTrace::assignRobDep(TraceInfo* past_record, TraceInfo* new_record) {
    DPRINTF(RxuElasticTrace, "%s %lli has ROB dependency on %lli\n",
...
bool
RxuElasticTrace::hasStoreCommitted(TraceInfo* past_record,
...
bool
RxuElasticTrace::hasLoadCompleted(TraceInfo* past_record,
                                    Tick execute_tick) const
{
...
bool
RxuElasticTrace::hasLoadBeenSent(TraceInfo* past_record,
                                Tick execute_tick) const
{
...
bool
RxuElasticTrace::hasCompCompleted(TraceInfo* past_record,
                                    Tick execute_tick) const
{
...
void
RxuElasticTrace::clearTempStoreUntil(const DynInstConstPtr& head_inst)
{
...
void
RxuElasticTrace::compDelayRob(TraceInfo* past_record, TraceInfo* new_record)
{
    ...
    DPRINTF(RxuElasticTrace, "Seq num %lli has ROB dependency on seq num %lli.\n",
    ...
    DPRINTF(RxuElasticTrace, "Computational delay is %lli - %lli = %lli\n",
    ...
    DPRINTF(RxuElasticTrace, "Final computational delay = %lli.\n",
            new_record->compDelay);
}

void
RxuElasticTrace::compDelayPhysRegDep(TraceInfo* past_record,
                                    TraceInfo* new_record)
{
    ...
    DPRINTF(RxuElasticTrace, "Seq. num %lli has register dependency on seq. num"
            " %lli.\n", new_record->instNum, past_record->instNum);
...
Tick
RxuElasticTrace::TraceInfo::getExecuteTick() const
{
    if (isLoad()) {
...
void
RxuElasticTrace::writeDepTrace(uint32_t num_to_write)
{
    ...
    while (num_to_write > 0) {
        ...
        if (!temp_ptr->isComp() || temp_ptr->numDepts != 0) {
            DPRINTFR(RxuElasticTrace, "Instruction with seq. num %lli "
                     "is as follows:\n", temp_ptr->instNum);
            if (temp_ptr->isLoad() || temp_ptr->isStore()) {
                DPRINTFR(RxuElasticTrace, "\tis a %s\n", temp_ptr->typeToStr());
                DPRINTFR(RxuElasticTrace, "\thas a request with phys addr %i, "
                         "size %i, flags %i\n", temp_ptr->physAddr,
            ...
            DPRINTFR(RxuElasticTrace, "\thas computational delay %lli\n",
                     temp_ptr->compDelay);
            ...
            if (temp_ptr->robDepList.empty()) {
                DPRINTFR(RxuElasticTrace, "\thas no order (rob) dependencies\n");
            }
            while (!temp_ptr->robDepList.empty()) {
                DPRINTFR(RxuElasticTrace, "\thas order (rob) dependency on %lli\n",
            ...
            if (temp_ptr->physRegDepList.empty()) {
                DPRINTFR(RxuElasticTrace, "\thas no register dependencies\n");
            }
            while (!temp_ptr->physRegDepList.empty()) {
                DPRINTFR(RxuElasticTrace, "\thas register dependency on %lli\n",
...
RxuElasticTrace::RxuElasticTraceStats::RxuElasticTraceStats(statistics::Group *parent)
...
const std::string&
RxuElasticTrace::TraceInfo::typeToStr() const
{
    return Record::RecordType_Name(type);
}

void
RxuElasticTrace::flushTraces()
{
