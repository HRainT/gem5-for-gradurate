#ifndef __CPU_RxuO3_DECODE_HH__
#define __CPU_RxuO3_DECODE_HH__
...
class Decode
{
  ...
  public:
    /** Decode constructor. */
    Decode(CPU *_cpu, const BaseRxuO3CPUParams &params);
    ...
    /** Sets pointer to time buffer coming from IBandLB. */
    ...
    /** Processes instructions from IBandLB and passes them on to predisq.
  ...
  private:
    ...
    /** Separates instructions from IBandLB into individual lists of instructions
     * sorted by thread.
     */
    void sortInsts();
    ...
    /** Returns if there any instructions from IBandLB on this cycle. */
    bool IBandLBInstsValid();
  ...
  private:
    ...
    // Might not be the best name as not only IBandLB will read it.
    TimeBuffer<TimeStruct>::wire toIBandLB;
    ...
    /** IBandLB instruction queue interface. */
    TimeBuffer<IBandLBStruct> *IBandLBQueue;

    /** Wire to get IBandLB's output from IBandLB queue. */
    TimeBuffer<IBandLBStruct>::wire fromIBandLB;

    /** Queue of all instructions coming from IBandLB this cycle. */
    std::list<DynInstPtr> insts[MaxThreads];

    /** Skid buffer between IBandLB and decode. */
    std::list<DynInstPtr> skidBuffer[MaxThreads];
    ...
    // -- add by hongfei.liu --------------------
    /** Predisq to decode delay. */
    Cycles predisqToDecodeDelay;
    // ------------------------------------------
    ...
    /** EW to decode delay. */
    Cycles ewToDecodeDelay;

    /** Commit to decode delay. */
    Cycles commitToDecodeDelay;

    /** IBandLB to decode delay. */
    Cycles IBandLBToDecodeDelay;
