#ifndef __CPU_RxuO3_LSQ_HH__
#define __CPU_RxuO3_LSQ_HH__
...
#include "enums/RxuSMTQueuePolicy.hh"
#include "mem/port.hh"
...
class LSQ
{
  public:
    ...
    LSQ(CPU *cpu_ptr, Dispipe3 *p3_ptr, const BaseRxuO3CPUParams &params);
    ...
    int getCountstore();
    /** Returns the number of instructions in the queues of one thread. */
    int getCountstore(ThreadID tid);

    /** Returns the number of instructions in all of the queues. */
    int getCountload();
    /** Returns the number of instructions in the queues of one thread. */
    int getCountload(ThreadID tid);
    ...
    /** The Dispipe3 stage pointer. */
    Dispipe3 *dispipe3Stage;

    /** The EW stage pointer. */
    EW *ewStage;
  ...
  public:
      /** Sets pointer to the EW. */
    void setEW(EW *ew_ptr) { ewStage = ew_ptr; }

    std::vector<DynInstPtr> requestVector;

    std::vector<LSQRequest*>::iterator itrequest;
    ...
    {
        LSQStats(CPU *cpu);
        statistics::Scalar instToTlb_num;
        statistics::Scalar instToTlb_cycle;
        statistics::Scalar ZeroCycle_num;
        statistics::Distribution instToTlb;
    } stats;
  ...
  protected:
     ...
     */
    static uint32_t
    maxLSQAllocation(RxuSMTQueuePolicy pol, uint32_t entries,
            uint32_t numThreads, uint32_t SMTThreshold)
    {
        if (pol == RxuSMTQueuePolicy::Dynamic) {
            return entries;
        } else if (pol == RxuSMTQueuePolicy::Partitioned) {
            //@todo:make work if part_amt doesnt divide evenly.
            return entries / numThreads;
        } else if (pol == RxuSMTQueuePolicy::Threshold) {
