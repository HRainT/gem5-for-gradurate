...
Dispipe3::Dispipe3Stats::Dispipe3Stats(CPU *cpu)
    ...
    ADD_STAT(LD_full, statistics::units::Count::get(),
             "Number of LDQueue  full "),
    ADD_STAT(ST_full, statistics::units::Count::get(),
             "Number of STQueue  full ")
{
    ...
    LD_full.prereq(LD_full);
    ST_full.prereq(ST_full);
}
...
Dispipe3::sendInsts(ThreadID tid)
{
    ...
    if (v_special_inst) {
        ...
        toEw->size++;
        DPRINTF(RxuDispipe3,
                "[tid:%i] Sending special Macro instruction [sn:%i] to ew, buffer size: %d.\n",
                tid, v_special_inst->seqNum, wtb.vSpecialBuffer->size());
    }
}
...
Dispipe3::canSplit(const InstQueue& uopqueue){
    ...
    for (size_t i = 0; i < uopqueue.size(); ++i){
        ...
        || uopqueue[i]->isVlse()
        || uopqueue[i]->isVsse() && (cpu->rmu.regScoreboard[src_reg2->flatIndex()] || 1)
        || uopqueue[i]->isVsseg() && cpu->rmu.regScoreboard[src_reg1->flatIndex()]
...
Dispipe3::eopReplace(DynInstPtr& inst)
{
    ...
    uint32_t             MicroIdx_vsseg    = inst->staticInst->getMicroIdx_vsseg();
    uint32_t             Field_x    = inst->staticInst->getField_x();
    ...
    dec_ptr->setMicroIdx_vsseg(MicroIdx_vsseg);
    dec_ptr->setField_x(Field_x);
...
void
Dispipe3::checkAndSplitEop(DynInstPtr& inst)
{
    for (auto it = vector_ldstQueue.begin(); it != vector_ldstQueue.end(); ++it) {
        ...
        if (canSplit(uopqueue)) {
            inst->inst_needEop_inMergeBuffer = true;
...
Dispipe3::insertLdStUop(DynInstPtr inst){
    bool stall = false;
    stall = ((156-ldstQueue.getCountstore(0)) <= 8) || ((256-ldstQueue.getCountload(0)) <= 8);
    if(stall){
    ...
    if (wtb.isFull(0, inst->ibuffer_id,isStore,memRef)) {
        ...
        if(inst->ibuffer_id == 24 || inst->ibuffer_id == 25 || inst->ibuffer_id == 26 || inst->ibuffer_id == 27) {
            ldst_wtb_full = true;
        }
        // block(0);
...
Dispipe3::insertInsts(ThreadID tid)
{
    ...
    for (int i = 0; i < insts_available; i++){
        ...
        // if(inst->isStore() && !inst->if_std_used_nolmal){
        //     inst->if_std_used_nolmal = true;
        //     continue;
        // }
    ...
    {   
        ...
        if(inst->isVector() && inst->isMacroVectorMemRef()){
            if(!cpu->mergeBuffer.entryEnough() && !inst->inst_needEop_inMergeBuffer){
                insts_from_dispipe2_in.push_back(inst);
                continue;
            }
            if(inst->needEop() && !inst->inst_needEop_inMergeBuffer){
                checkAndSplitEop(inst);
            }
            if(inst->isSquashed()){
        ...
        if(inst->isMemRef() && (ldst_wtb_full || (!ldst_vector_ready) || (!cpu->mergeBuffer.entryEnough()))){
            insts_from_dispipe2_in.push_back(inst);
            continue;
        }
        ...
        if(!inst->isSquashed() && !inst->inst_had_insertq && (deep_eo - all_cnt) > 0){
            bool stall = false;
            stall = ((156-ldstQueue.getCountstore(tid)) <= 8) || ((256-ldstQueue.getCountload(tid)) <= 8);
        ...
        if (wtb.isFull(tid, inst->ibuffer_id,isStore,memRef)) {
            ...
            inst->ibuffer_id == 24 || inst->ibuffer_id == 25 || 
            inst->ibuffer_id == 26 || inst->ibuffer_id == 27) {
                ldst_wtb_full = true;
            }
            if(inst->ibuffer_id == 0 || inst->ibuffer_id == 1 ||
...
Dispipe3::tick()
{
    ...
    if(((156-ldstQueue.getCountstore(0)) <= 8)){
        stats.ST_full++;
    }
    else if(((256-ldstQueue.getCountload(0)) <= 8)){
        stats.LD_full++;
    }
    ...
    while (threads != end) {
        ...
        wtb.stats.ibuffer15Utilize += wtb.ibuffer[tid][6].size();
        wtb.stats.ibuffer16Utilize += wtb.ibuffer[tid][7].size();
        wtb.stats.ibuffer17Utilize += wtb.ibuffer[tid][6].size();
        wtb.stats.ibuffer18Utilize += wtb.ibuffer[tid][7].size();
        wtb.stats.ibuffer19Utilize += wtb.ibuffer[tid][6].size();
        wtb.stats.ibuffer20Utilize += wtb.ibuffer[tid][7].size();
        wtb.stats.ibuffer21Utilize += wtb.ibuffer[tid][6].size();
        wtb.stats.ibuffer22Utilize += wtb.ibuffer[tid][22].size();
        wtb.stats.ibuffer23Utilize += wtb.ibuffer[tid][23].size();
        // wtb.stats.ibuffer24Utilize += wtb.ibuffer[tid][6].size();
        // wtb.stats.ibuffer25Utilize += wtb.ibuffer[tid][7].size();
        // wtb.stats.ibuffer26Utilize += wtb.ibuffer[tid][6].size();
        // wtb.stats.ibuffer27Utilize += wtb.ibuffer[tid][7].size();
