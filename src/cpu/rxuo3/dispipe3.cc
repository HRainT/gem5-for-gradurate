...
Dispipe3::checkStall(ThreadID tid)
{
    ...
    if ((112-ldstQueue.getCountstore(tid)) <= 8) {
        DPRINTF(RxuDispipe3, "storequeue may be becoming full.\n");
        ret_val = true;
    }

    if ((164-ldstQueue.getCountload(tid)) <= 8) {
        DPRINTF(RxuDispipe3, "loadqueue may be becoming full.\n");
        ret_val = true;
    }
...
Dispipe3::eopReplace(DynInstPtr& inst)
{
    ...
    "[sn:%i] Begin split eop.\n", inst->seqNum);
        ...
        DPRINTF(RxuDispipe3,
        "Uop[sn:%i]: Eop [sn:%i] had been split,uopNumber = %i,uopNumber_notWtb = %i .\n", 
        inst->seqNum,instruction->seqNum,inst->ori_inst->uopNumber,inst->ori_inst->uopNumber_notWtb);
    ...
    for (int i = 0; i < insts_available_in; i++)
    {
        ...
        if(inst->isVector() && inst->isMacroVectorMemRef()){
            if(inst->isSquashed()){
                ...
                {
                    if(inst->isMemRef() && (ldst_wtb_full || !ldst_vector_ready) && inst->isMicroop()){
                        insts_from_dispipe2_in.push_back(inst);
                        break;
                    }
        ...
        if (wtb.isFull(tid, inst->ibuffer_id,isStore,memRef)) {
            ...
            // int size = wtb.ibuffer[tid][inst->ibuffer_id].size();
            // DynInstPtr inst_full;
            // for(int i =0; i<size; i++){
            //     inst_full = wtb.ibuffer[tid][inst->ibuffer_id].front();
            //     wtb.ibuffer[tid][inst->ibuffer_id].pop_front();
            //     DPRINTF(RxuLDST, "inst full seqnum:%i",inst_full->seqNum);
            //     wtb.ibuffer[tid][inst->ibuffer_id].push_back(inst_full);
            // }
            if(inst->isSquashed()){
