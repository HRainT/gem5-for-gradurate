#include "cpu/rxuo3/inst_queue.hh"
      ...
      iqIOStats(cpu)
{
    ...
    if (iqPolicy == RxuSMTQueuePolicy::Dynamic) {
        //Set Max Entries to Total ROB Capacity
        for (ThreadID tid = 0; tid < numThreads; tid++) {
            maxEntries[tid] = numEntries;
        }

    } else if (iqPolicy == RxuSMTQueuePolicy::Partitioned) {
        ...
        DPRINTF(RxuIQ, "IQ sharing policy set to Partitioned:"
                "%i entries per thread.\n",part_amt);
    } else if (iqPolicy == RxuSMTQueuePolicy::Threshold) {
        ...
        DPRINTF(RxuIQ, "IQ sharing policy set to Threshold:"
                "%i entries per thread.\n",thresholdIQ);
   }
...
{
    if (iqPolicy == RxuSMTQueuePolicy::Partitioned) {
...
{
    if (iqPolicy != RxuSMTQueuePolicy::Dynamic || numThreads > 1) {
...
InstructionQueue::insert(const DynInstPtr &new_inst)
{
    ...
    DPRINTF(RxuIQ, "Adding instruction [sn:%llu] PC %s to the IQ.\n",
...
InstructionQueue::insertNonSpec(const DynInstPtr &new_inst)
{
    ...
    DPRINTF(RxuIQ, "Adding non-speculative instruction [sn:%llu] PC %s "
            "to the IQ.\n",
            new_inst->seqNum, new_inst->pcState());
...
{
    DPRINTF(RxuIQ, "Processing FU completion [sn:%llu]\n", inst->seqNum);
...
{
    DPRINTF(RxuIQ, "Attempting to schedule ready instructions from "
            "the IQ.\n");
    ...
    while (total_issued < totalWidth && order_it != order_end_it) {
        ...
        if (idx != RxuFUPool::NoFreeFU) {
            ...
            DPRINTF(RxuIQ, "Thread %i: Issuing instruction PC %s "
                    "[sn:%llu]\n",
    ...
    if (total_issued || !retryMemInsts.empty() || !deferredMemInsts.empty()) {
        ...
        DPRINTF(RxuIQ, "Not able to schedule any instructions.\n");
    }
}
...
{
    DPRINTF(RxuIQ, "Marking nonspeculative instruction [sn:%llu] as ready "
            "to execute.\n", inst);
...
{
    DPRINTF(RxuIQ, "[tid:%i] Committing instructions older than [sn:%llu]\n",
            tid,inst);
...
InstructionQueue::wakeDependents(const DynInstPtr &completed_inst)
{
    ...
    DPRINTF(RxuIQ, "Waking dependents of completed instruction.\n");
    ...
    if (completed_inst->isMemRef()) {
        ...
        DPRINTF(RxuIQ, "Completing mem instruction PC: %s [sn:%llu]\n",
         ...
         dest_reg_idx++)
    {
        ...
        if (dest_reg->isFixedMapping()) {
            DPRINTF(RxuIQ, "Reg %d [%s] is part of a fix mapping, skipping\n",
        ...
        if (dest_reg->getNumPinnedWritesToComplete() != 0) {
            DPRINTF(RxuIQ, "Reg %d [%s] is pinned, skipping\n",
                    dest_reg->index(), dest_reg->className());
            continue;
        }

        DPRINTF(RxuIQ, "Waking any dependents on register %i (%s).\n",
                dest_reg->index(),
                dest_reg->className());
        ...
        while (dep_inst) {
            DPRINTF(RxuIQ, "Waking up a dependent instruction, [sn:%llu] "
...
InstructionQueue::addReadyMemInst(const DynInstPtr &ready_inst)
{
    ...
    DPRINTF(RxuIQ, "Instruction is ready to issue, putting it onto "
...
{
    DPRINTF(RxuIQ, "Rescheduling mem inst [sn:%llu]\n", resched_inst->seqNum);
...
InstructionQueue::blockMemInst(const DynInstPtr &blocked_inst)
{
    ...
    DPRINTF(RxuIQ, "Memory inst [sn:%llu] PC %s is blocked, will be "
            "reissued later\n", blocked_inst->seqNum,
...
{
    DPRINTF(RxuIQ, "Cache is unblocked, rescheduling blocked memory "
            "instructions\n");
...
{
    DPRINTF(RxuIQ, "[tid:%i] Starting to squash instructions in "
            "the IQ.\n", tid);
...
InstructionQueue::doSquash(ThreadID tid)
{
    ...
    DPRINTF(RxuIQ, "[tid:%i] Squashing until sequence number %i!\n",
    ...
    while (squash_it != instList[tid].end() &&
        ...
        if (!squashed_inst->isIssued() ||
            ...
            DPRINTF(RxuIQ, "[tid:%i] Instruction [sn:%llu] PC %s squashed.\n",
...
InstructionQueue::addToProducers(const DynInstPtr &new_inst)
{
         ...
         dest_reg_idx++)
    {
        ...
        // dependGraph.insert(dest_reg->flatIndex(), new_inst);
...
InstructionQueue::addIfReady(const DynInstPtr &inst)
{
    ...
    if (inst->readyToIssue()) {
        ...
        if (inst->isMemRef()) {
            ...
            DPRINTF(RxuIQ, "Checking if memory instruction can issue.\n");
