...
//    activity = false;
    ...
    if (curCycle() % 10000 == 0 && curCycle() > 0) {
        cprintf("curCycle(): %llu,curTick(): %llu \n", curCycle(),curTick());
...
CPU::MergeBuffer::MergeBuffer(int maxEntryNum)
            : maxEntryNum(maxEntryNum)
        {}

void
CPU::MergeBuffer::addEntry(InstSeqNum sn, DynInstPtr inst, int eop_num) {
    DPRINTF(RxuO3CPU, "[sn:%lli] Uop add to merge buffer\n", sn);
    RiscvISA::VecRegContainer *data = new RiscvISA::VecRegContainer;
    mergeBufferEntry buffer_entry = {data, eop_num, false, inst};
    Buffer[sn] = buffer_entry;
}

int
CPU::MergeBuffer::getSize(){
    return Buffer.size(); 
}

mergeBufferEntry
CPU::MergeBuffer::checkAndFetchEntry() {
    if (Buffer.empty()) return  {nullptr, 0, false, nullptr};
    int cnt = 0;
    for (auto it : Buffer) {
        cnt++;
        if(cnt <=10){
            DPRINTF(RxuO3CPU, "merge buffer entry: [sn:%lli], remain unmerge eop: %d\n", 
            it.first, it.second.remainUnmergeEop);
        }
        else if(cnt == 11){
            DPRINTF(RxuO3CPU, "merge buffer entry >10 \n");
        }
        else {
            continue;
        }
    ...
    for (auto it = Buffer.begin(); it != Buffer.end(); ++it) {
        if (it->second.remainUnmergeEop == 0) {
            mergeBufferEntry entry = it->second;
            it = Buffer.erase(it);
            if (entry.inst->isSquashed()) {
                return {nullptr, 0, false, nullptr};
            }
            return entry;
        }
    }
    DPRINTF(RxuO3CPU, "merge buffer all inst not ready\n");
    return {nullptr, 0, false, nullptr};
}

bool
CPU::MergeBuffer::entryEnough() {
    return (Buffer.size()) <= maxEntryNum;
}

RiscvISA::VecRegContainer *
CPU::MergeBuffer::fetchEntry(InstSeqNum sn) {
    assert(Buffer.find(sn) != Buffer.end());
    RiscvISA::VecRegContainer *data = Buffer[sn].data;
    DPRINTF(RxuO3CPU,
            "[sn:%lli] Uop's eop write data to merge buffer, old data: %s, remain unmerge eop: %d\n", 
            sn, reverseByteOrder(data->getString(16)), Buffer[sn].remainUnmergeEop-1);
    return data;
}

bool
CPU::MergeBuffer::alreadySetOldVd(InstSeqNum sn) {
    if (!Buffer[sn].alreadySetOldVd) {
        Buffer[sn].alreadySetOldVd = true;
        return false;
    }
    return true;
}
...
void
CPU::MergeBuffer::decreaseEntryEopNum(InstSeqNum sn) {
    Buffer[sn].remainUnmergeEop--;
}

void
CPU::MergeBuffer::squash(InstSeqNum squashed_sn) {
    for(auto it = Buffer.begin(); it != Buffer.end(); ) {
        if (it->first >= squashed_sn) {
            it->second.inst->setSquashed();
            DPRINTF(RxuO3CPU,
            "[sn:%lli] Uop's eop in merge buffer, been squashed  \n", it->second.inst->seqNum);
            // it = Buffer.erase(it);
            it++;
        } else {
            it++;
...
std::string
CPU::MergeBuffer::name()
{
    return "MergeBuffer";
}

std::string
CPU::MergeBuffer::reverseByteOrder(const std::string& input) {
    // 去除方括号
