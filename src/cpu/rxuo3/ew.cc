...
namespace gem5
{
    ...
    namespace rxuo3
    {
        ...
        EW::instToCommit(const DynInstPtr &inst)
        {
            ...
            {   
                ...
                {   
                    ...
                    cpu->schedule(Vector, cpu->clockEdge(Cycles(op_latency-1)));
                }
                else {
                    int sendCycle = -1;
        ...
        EW::processLoadStoreLatency(const DynInstPtr &inst)
        {
            ...
            if(inst->staticInst->opClass() == SimdVLoadOnceOp){
                    PhysRegIdPtr phys_reg = inst->renamedDestIdx(0);
                    char dest_val = 0;
                    dest_val = cpu->getVectorReg(phys_reg);
                    cpu->prefetch_load[inst->load_once] = dest_val;
                    DPRINTF(RxuEW, "[sn:%llu] loadonce instruction dest(data):%llx,address is%llx.\n",inst->seqNum, 
                    dest_val,inst->load_once);
                }
