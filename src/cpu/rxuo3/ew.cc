    ...
    namespace rxuo3
    {
        ...
        EW::squashDueToBranch(const DynInstPtr &inst, ThreadID tid)
        {
                ...
                inst->seqNum < toCommit->squashedSeqNum[tid])
            {
                ...
                if (inst->isVset() && inst->has_ori_inst) {
        ...
        EW::instToCommit(const DynInstPtr &inst)
        {
            ...
            // free slot.
            if (!inst->ifstdataready && inst->isStore() && !inst->tlbfault && !inst->isStoreConditional())
            {
                return;
            }
            ...
            {
                while ((*ewQueue)[wbCycle].insts[wbNumInst])
                {
                    ++wbNumInst;
                ...
                (*ewQueue)[wbCycle].insts[wbNumInst] = inst;
                (*ewQueue)[wbCycle].size++;
            ...
            else
            {
                ...
                {
                    cpu->schedule(LDST, cpu->clockEdge(Cycles(4)));
                }
                else
                {
                    cpu->schedule(LDST, cpu->clockEdge(Cycles(3)));
                }
        ...
        EW::processLoadStoreLatency(const DynInstPtr &inst)
        {
            ...
            wakeCPU();
            DPRINTF(RxuEW,
                    "load/store instruction [sn:%llu] with PC %s is executing DC3, ready to WB/commit next cycle.\n",
                    inst->seqNum, inst->pcState());
                ...
                if (inst->isMemRef())
                {
                    ...
                    else if (inst->isStore())
                    {
                            ...
                            inst->tlbfault)
                        {
                            ...
                            if (!(inst->if_exe_second && inst->num_exe != 2) || inst->isNonSpeculative() || fault != NoFault || inst->tlbfault)
                            {
                                inst->setExecuted();
                                instToCommit(inst);
                                activityThisCycle();
        ...
        EW::writebackInsts()
        {
            ...
            for (int inst_num = 0; inst_num < size; inst_num++)
            {
                ...
                ThreadID tid = inst->threadNumber;
                if(inst->isEop()) {
                    cpu->dispipe3.wtb.memDepUnit[tid].completed(inst);
                }
            ...
            while (1) {
                ...
                DynInst *inst = merge_buffer_entry.inst;
        ...
        EW::WriteFrm(DynInstPtr inst, ThreadID tid)
        {
            ...
            if ((inst_name == "csrrsi" || inst_name == "csrrci") && (imm == 0))
            {
                return;
            }
        ...
        void
        EW::WriteCsrVlVtype(DynInstPtr inst, ThreadID tid)
        {
            RegVal imm = inst->staticInst->machInst.vecimm;
            std::string inst_name = inst->staticInst->getName();
