...
class DynInst : public ExecContext, public RefCounted
{
  ...
  public:
    ...
    // if ta ma
    bool if_ta_ma = true;
    //if std use int normal
    bool if_std_used_nolmal = false;
    //if had split eop && in mergebuffer
    bool inst_needEop_inMergeBuffer = false;
  ...
  public:
    ...
    mispredicted()
    {
        ...
        DPRINTF(RxuDynInst,
                "npc vl = %d, tpc vl = %d\n",
                (*next_pc).as<gem5::RiscvISA::PCState>()._vl,
                (*predPC).as<gem5::RiscvISA::PCState>()._vl);
        bool mispred = false;
...
// test for eop
    ...
    bool isVIndexORStrideStore() const {
        return (opClass() == enums::SimdIndexedStore || opClass() == enums::SimdStridedStore || opClass() == enums::SimdUnitStrideSegmentedStore
 ...
 // Value -1 indicates that particular phase
    ...
    readMiscReg(int misc_reg) override
    {
        ...
        if (needVxrm() && misc_reg == RiscvISA::MISCREG_VXRM && rn_src_vxrm != 0)
            return vxrmVal();
        if (!cpu->vsetBranch) {
            if (isCsrVl() && misc_reg == RiscvISA::MISCREG_VL)
                return vlVal();
            if (isCsrVtype() && misc_reg == RiscvISA::MISCREG_VTYPE)
                return vtypeVal();
        }
