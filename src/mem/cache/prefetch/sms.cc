...
#include "debug/stride_prefetch.hh"
#include "debug/stream_prefetch.hh"
#include "debug/sms_prefetch.hh"
#include "debug/StideSinglePrefetcher.hh"
#include "debug/StideSinglePrefetcherTrans.hh"
                                 ...
                                 PrefetchSourceType pf_source, bool miss_repeat)
{
    ...
    if(old_curtick == 0 || old_curtick == curTick()){
        pc_load[cnt] = (pfi.getPC()>>2) & 0b1111111;
        cnt++;
    }
    else {
        old_curtick = curTick();
        if(cnt == 0){
            DPRINTF(XSCompositePrefetcher, "no prefetch or only one prefetch in previous circle\n");
        }
        else if(cnt == 1){
            if(pc_load[0] == pc_load[1]){
                stats.pc8to2_same_ld++;
            }
        }
        else if(cnt == 2){
            if(pc_load[0] == pc_load[1] || pc_load[0] == pc_load[2] ||  pc_load[2] == pc_load[1]){
                stats.pc8to2_same_ld++;
            }
        }
        else if(cnt == 3){
            if(pc_load[0] == pc_load[1] || pc_load[0] == pc_load[2] || pc_load[0] == pc_load[3]
                || pc_load[1] == pc_load[2] || pc_load[1] == pc_load[3] || pc_load[2] == pc_load[3]){
                stats.pc8to2_same_ld++;
            }
        }
        cnt = 0;
            ...
            if (is_active_page) {
                ...
                if (decr) {
                    ...
                    for (int i = regionBlks - 1; i >= 0; i--) {
                        ...
                        DPRINTF(stream_prefetch, "stream pf addr: %x [%d]\n", cur, i);
                        fatal_if(i < 0, "i < 0\n");
                    }
                } else {
    ...
    if (act_match_entry && is_active_page && pf_tgt_addr && enter_new_region) {
        if (streamPFAhead) {
            ...
            for (int i = 0; i < regionBlks; i++) {
                ...
                DPRINTF(stream_prefetch, "stream L2 pf addr: %x [%d]\n", cur, i);
            }
    ...
    if(pfi.getPC() == 0x20924 || pfi.getPC() == 0x20a3c || pfi.getPC() == 0x141c0 || pfi.getPC() == 0x141d0){
        DPRINTF(StideSinglePrefetcherTrans, "load pc:%x,vaddr:%x\n",pfi.getPC(),pfi.getAddr());
    }
                                 ...
                                 bool &is_first_shot)
{
    ...
    if (entry) {
        // act hit
        entry->new_region_offset = region_offset;
        if(entry->new_region_offset > entry->region_offset){
            entry->cnt = +1;
        }
        else if(entry->new_region_offset < entry->region_offset){
            entry->cnt = -1;
        }
    ...
    entry->access_cnt = 1;
    act.insertEntry(region_addr, secure, entry,0);
...
XSCompositePrefetcher::markNonStridePC(Addr pc)
{
    ...
    if (entry) {
        ...
        assert(entry);
        entry->pc = pc;
        nonStridePCs.insertEntry(nonStrideHash(pc), false, entry,0);
    }
}
    ...
    if (entry) {
        ...
        if (entry->conf >= 2) {
            ...
            for (unsigned i = start_depth; i <= entry->depth; i++) {
                ...
                DPRINTF(StideSinglePrefetcher, "Load PC:%x Stride conf >= 2, send pf: %x with depth %i\n",entry->pc, pf_addr, i);
            }
        ...
        stride.insertEntry(pfi.getPC(), pfi.isSecure(), entry,0);
    }
...
XSCompositePrefetcher::updatePht(XSCompositePrefetcher::ACTEntry *act_entry, Addr current_region_addr,bool re_act_mode)
{
    ...
    if (!is_update) {
        ...
        pht.insertEntry(phtHash(act_entry->pc, act_entry->region_offset), act_entry->is_secure, pht_entry,0);
    } else {
    ...
    if (pht_entry) {
        ...
        for (uint8_t i = 0; i < regionBlks - 1; i++) {
            if (pht_entry->hist[i + regionBlks - 1].calcSaturation() > 0.5) {
                ...
                DPRINTF(stream_prefetch, "PHT stream  pf addr: %x [%d]\n", pf_tgt_addr, i);
                found = true;
            }
        }
                                        ...
                                        PrefetchSourceType src, int ahead_level)
{
    if (ahead_level < 2 && pfPageLRUFilter.contains(regionAddress(addr))) {
        ...
        DPRINTF(sms_prefetch, "sms pf addr: %x [%d]\n", addr, src);
        if (ahead_level > 1) {
...
XSCompositePrefetcher::XSCompositeStats::XSCompositeStats(statistics::Group *parent)
    ...
    ADD_STAT(pc8to2_same_ld,statistics::units::Count::get(),"pc8_2 same ld in the same circle"),
    ADD_STAT(all_prefetch_number,statistics::units::Count::get(),"all number of prefetch")
    {
    }
