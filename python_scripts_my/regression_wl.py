...
default_output_root  = os.path.join("./out", "randomx", today+'merge_public_itage')  # 构建目录路径
...
simulation_cases = [
   {
    ...
    # },
    # {
    #     'kernel': '/Data/zhuohao.zhang/case/linpack/linpack_size20x20_loop100_testcase_rolling/linpack_size20x20_loop100_testcase_rolling.elf',
    #     # 其他需要覆盖的参数...
    # },
    # {
    #     'kernel': '/Data/longting.du/case/linpack/linpack_size20x20_loop100_testcase_unrolling/linpack_size20x20_loop100_testcase.elf',
    #     # 其他需要覆盖的参数...
    # },
    # {
    #     'kernel': '/Data/zhuohao.zhang/case/linpack/linpack_size100x100_loop5_testcase_rolling/linpack_size100x100_loop5_testcase_rolling.elf',
    #     # 其他需要覆盖的参数...
    # },
    # {
    #     'kernel': '/Data/longting.du/case/linpack/linpack_size100x100_loop5_testcase_unrolling/linpack_size100x100_loop5_testcase.elf',
    #     # 其他需要覆盖的参数...
    # },
    # {
...
stat_mappings = {
    ...
    "system.cpu.fetch.CAMHitCount": "CAMHitCount",
...
def generate_tables(simulation_cases, default_output_root, stat_mappings):
    ...
    for case in simulation_cases:
        ...
        if os.path.getsize(stats_file) > 0:
            ...
            data_row = [kernel_name] + [data.get(key, "nan") for key in stat_mappings.keys()]
            data_for_df.append(data_row)
           # 收集IPC值
            ipc_value = data.get("system.cpu.ipc301_400loop", "nan")
            if ipc_value != "nan":
                ipc_values.append(float(ipc_value))
        else:
            print(f"Error: The .stat file for {kernel_name} is empty.")

   # 计算几何平均
    if ipc_values:
        geometric_mean_ipc = np.exp(np.mean(np.log(ipc_values)))
        # 格式化几何平均值为小数点后六位
        formatted_geometric_mean_ipc = f"{geometric_mean_ipc:.6f}"
    else:
        formatted_geometric_mean_ipc = "nan"

    # 创建DataFrame
    df = pd.DataFrame(data_for_df, columns=['Kernel'] + list(stat_mappings.values()))
    df.loc['Geometric Mean'] = ['Geometric Mean'] + [''] * (len(df.columns) - 2) + [formatted_geometric_mean_ipc]
    # 确保输出目录存在
